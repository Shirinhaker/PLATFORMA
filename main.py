...
<title>Platforma</title><!-- BUILD: v0639 -->
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent;}
  ...
  /* CHAT */
  .msg{max-width:78%;padding:9px 13px;border-radius:16px;margin-bottom:8px;font-size:14.5px;line-height:1.4;word-wrap:break-word;white-space:pre-wrap;}
  ...
  <div class="phone">
    ...
    <main class="screens" id="screens">
      ...
      <section class="screen" data-screen="login">
        <div class="form-wrap">
          ...
          <div id="loginStep1">
            ...
            <div class="form-foot" style="margin-top:10px"><b id="forgotPass" style="color:var(--primary)">Parolni unutdingizmi?</b></div>
      ...
      <section class="screen" data-screen="chat">
        <div id="chatThread" style="padding:10px 14px 90px;min-height:calc(100vh - 200px)"></div>
        <div class="chat-bar">
...
(function(){
  ...
  function selectMap(key){
    ...
    if(key==="biz"){
      renderPins("biz"); renderMapChip(); renderElonRow(); el("elonList").innerHTML="";
      return;
    }
  ...
  function submitRegister(){
    ...
    api("POST", "/api/auth/register", body).then(function(r){
      // Login va parolni foydalanuvchiga ko'rsatamiz (Telegramga ham yuborilgan)
  ...
  function showCredentials(login, password, role){
    var isReset = !role;
    el("regBody").innerHTML =
      '<div class="form-wrap"><div class="lead">'+(isReset?"Parol yangilandi! ✅":"Ro'yxatdan o'tdingiz! ✅")+'</div>'+
      ...
      '</div>'+
      '<button class="btn btn-primary btn-block" id="credDone">'+(isReset?"Kirishga o'tish":"Kabinetimga o'tish")+'</button>'+
      '</div>';
    nav("register");
    el("tbTitle").textContent = isReset ? "Yangi parol" : "Ro'yxatdan o'tish";
    el("credDone").addEventListener("click", function(){
      if(isReset){
        // login ekraniga qaytamiz, loginni avtomatik to'ldiramiz
        if(el("loginUser")) el("loginUser").value = login;
  ...
  function openChat(userId, name){
    ...
    chatPeer = { id:userId, name:name||"Suhbat" };
    chatBack = current;
    el("tbTitle").textContent = name || "Suhbat";
  ...
  function loadChat(scroll){
    if(!chatPeer) return;
    api("GET","/api/messages/with/"+chatPeer.id).then(function(d){
  ...
  function renderChatThread(msgs){
    ...
    msgs.forEach(function(m){
      ...
      html += '<div class="msg '+(m.mine?"me":"them")+'">'+esc(m.text)+'<span class="msg-time">'+t+'</span></div>';
    });
  ...
  function sendChat(){
    ...
    inp.value="";
    api("POST","/api/messages/send",{to:chatPeer.id, text:text}).then(function(){
      loadChat(true);
    }).catch(function(e){ alert(e.message); inp.value=text; });
  }
  function openChats(){
    ...
    nav("chats");
    el("tbTitle").textContent = "Suhbatlar";
    api("GET","/api/messages/conversations").then(function(list){
      ...
      el("chatsList").innerHTML = list.map(function(c){
        return '<div class="conv" data-chat="'+c.user_id+'" data-name="'+esc(c.name)+'">'+
  ...
  function openPersonSrv(id){
    api("GET","/api/person/"+id).then(function(p){
      ...
      bindPageFollow();
      var pm = el("pageMsg");
      if(pm) pm.addEventListener("click", function(){ openChat(p.id, p.name); });
    }).catch(function(e){ alert(e.message); });
  }
  ...
  el("locAuto").addEventListener("click", function(){
    ...
    navigator.geolocation.getCurrentPosition(function(pos){
      ...
      CENTER=[lat,lng];
      if(LMAP){ try{ LMAP.setView(CENTER, 15); }catch(e){} }
      // manzil nomini OpenStreetMap orqali aniqlaymiz (teskari geokodlash)
      fetch("https://nominatim.openstreetmap.org/reverse?format=json&lat="+lat+"&lon="+lng+"&accept-language=uz")
        .then(function(r){ return r.json(); })
        .then(function(d){
          var a=d.address||{};
          var place = a.suburb || a.neighbourhood || a.city_district || a.county || a.city || a.town || a.village || "";
          LOC = place ? place : "Joylashuvim";
          renderMapChip(); done(); nav("home");
        })
        .catch(function(){
          LOC = "Joylashuvim"; renderMapChip(); done(); nav("home");
        });
    }, function(err){
      done();
  ...
  el("locSave").addEventListener("click", function(){
    ...
    LOC = s.tuman + (mahalla ? (", "+mahalla) : "");
    CENTER = [s.lat, s.lng];
    if(LMAP){ try{ LMAP.setView(CENTER, 13); }catch(e){} }
    renderMapChip();
    nav("home");
  });
  ...
  el("elonList").addEventListener("click", function(e){
    ...
    if(sv){
      ...
      api("POST","/api/save",{target_kind:"listing",target_id:lid}).then(function(r){
  ...
  el("chatsList").addEventListener("click", function(e){
    var c=e.target.closest("[data-chat]"); if(c) openChat(parseInt(c.getAttribute("data-chat"),10), c.getAttribute("data-name"));
  });
  ...
  el("forgotPass").addEventListener("click", function(){
    if(!confirm("Yangi parol yaratilib, Telegramingizga yuboriladi. Davom etamizmi?")) return;
    var b=this; b.style.pointerEvents="none"; b.textContent="Yuborilmoqda...";
    api("POST","/api/auth/reset",{}).then(function(r){
      showCredentials(r.login, r.password, null);
      el("tbTitle").textContent="Yangi parol";
    }).catch(function(e){ alert(e.message); })
      .finally(function(){ b.style.pointerEvents=""; b.textContent="Parolni unutdingizmi?"; });
  });
  ...
  function onScreenOpen(screen){
    ...
    else if(screen==="loc") fillRegionSelects();
  }
  ...
  var PMAP = null;
  var pickedLatLng = { be:null, ue:null };   // har forma uchun tanlangan koordinata
  ...
  function openPicker(prefix){
    ...
    nav("pickloc");
    setTimeout(function(){
      var startC = pickedLatLng[prefix] || CENTER || [41.311, 69.280];
      if(!PMAP){
  ...
  el("pickConfirm").addEventListener("click", function(){
    if(PMAP && pickTarget){
      ...
      setPicked(pickTarget, c.lat, c.lng);
    }
    nav(pickTarget==="be" ? "cab-elon-form" : "ucab-elon-form");
  });
  ...
  function bindElonForm(prefix, listId, catsId, getVisibility){
    ...
    el(prefix+"Submit").addEventListener("click", function(){
      ...
      var body={ cat:cat, title:title, price:(el(prefix+"Price").value||"").trim(),
                 ...
                 lat: ll[0], lng: ll[1], media:selMedia.slice() };
  ...
  function bootWhenReady(){
    ...
    if(typeof L === "undefined"){
      ...
      var iv = setInterval(function(){
        ...
        if(typeof L !== "undefined"){
          clearInterval(iv);
          try { ensureMap(); renderPins(mapMode || "biz"); } catch(e){}
