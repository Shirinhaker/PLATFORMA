...
$ExpectedHead = "0042_real_business_subscriptions"

Write-Host "SCRIPT_VERSION=9"
Write-Host "MIGRATION_MODE=PROMOTE_VERIFIED_V9_CANDIDATE"
Write-Host ("EXECUTE={0}" -f $Execute.IsPresent)
Write-Host ("APPROVED_STAGING_RUN_ID={0}" -f $ApprovedStagingRunId)

if ($SnapshotPath -notmatch '^/tmp/koprik-phase3c-v9-[0-9]{8}-[0-9]{6}/snapshot/platforma\.snapshot\.db$') {
    throw "PRODUCTION_V9_SNAPSHOT_PATH_NOT_FROM_STAGING_WORKDIR"
}
if ($ExpectedSnapshotSha256 -notmatch '^[0-9a-fA-F]{64}$') {
    throw "PRODUCTION_V9_SNAPSHOT_SHA256_INVALID"
}
if ($ExpectedManifestSha256 -notmatch '^[0-9a-fA-F]{64}$') {
    throw "PRODUCTION_V9_MANIFEST_SHA256_INVALID"
}
...
function Invoke-RemoteBash {
    ...
    $InputPath = Join-Path $TempRoot "koprik-v9-production-$Token.sh"
    $OutputPath = Join-Path $TempRoot "koprik-v9-production-$Token.out"
    $ErrorPath = Join-Path $TempRoot "koprik-v9-production-$Token.err"
...
fail() {
  printf 'PHASE3C_V9_PRODUCTION_ERROR=%s\n' "$1" >&2
  exit 1
}
...
EXPECTED_HEAD="0042_real_business_subscriptions"
...
printf 'PRODUCTION_V9_PROMOTION_GUARD_OK STAGING_RUN_ID=%s\n' \
...
$PreflightOutput = Invoke-RemoteBash `
    ...
    -FailureCode "PRODUCTION_V9_PREFLIGHT_FAILED"
if (($PreflightOutput -join "`n") -notmatch "PRODUCTION_V9_PROMOTION_GUARD_OK") {
    throw "PRODUCTION_V9_GUARD_CONFIRMATION_MISSING"
}
...
if (-not $BackupConfirmed.IsPresent) {
    throw "PRODUCTION_V9_BACKUP_CONFIRMATION_REQUIRED"
}
if (-not $MaintenanceConfirmed.IsPresent) {
    throw "PRODUCTION_V9_MAINTENANCE_CONFIRMATION_REQUIRED"
}
if (-not $SourceWritesStoppedConfirmed.IsPresent) {
    throw "PRODUCTION_V9_SOURCE_WRITES_STOPPED_CONFIRMATION_REQUIRED"
}
...
assert report["schema_version"] == "0009_phase3c_real_subscriptions_v1", report["schema_version"]
...
printf 'PHASE3C_V9_PRODUCTION_PROMOTION_COMPLETE RUN_ID=%s STAGING_RUN_ID=%s WORK=%s\n' \
...
Invoke-RemoteBash `
    ...
    -FailureCode "PRODUCTION_V9_PROMOTION_FAILED" | Out-Null

Write-Host "Phase 3C V9 production data promotion finished."
