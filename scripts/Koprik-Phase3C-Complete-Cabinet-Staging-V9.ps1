...
$ExpectedSchema = "0009_phase3c_real_subscriptions_v1"
$ExpectedInitialHead = "0005_profile_cabinet_parity"
$ExpectedFinalHead = "0042_real_business_subscriptions"
$RemoteRoot = "/tmp/koprik-phase3c-v9-input"
...
function Invoke-RemoteBash {
    ...
    $InputPath = Join-Path $TempRoot "koprik-v9-$Token.sh"
    $OutputPath = Join-Path $TempRoot "koprik-v9-$Token.out"
    $ErrorPath = Join-Path $TempRoot "koprik-v9-$Token.err"
...
test "$DEPLOYED_SCHEMA" = "0009_phase3c_real_subscriptions_v1" \
...
printf '%s' "$ALEMBIC_HEADS" | grep -q "0042_real_business_subscriptions" \
...
from app.legacy_migration.real_source_v9 import PRESERVE_DEMO_FLAG_TABLES
...
assert "business_subscriptions" in PRESERVE_DEMO_FLAG_TABLES
assert callable(import_business_subscriptions)
assert callable(import_late_typed_domains)
print("CURRENT_TYPED_CABINET_CODE_GUARD_OK")
PY

printf 'STAGING_V9_GUARD_OK SCHEMA=%s INITIAL_HEAD=%s FINAL_HEAD=%s BACKEND=%s\n' \
  "$DEPLOYED_SCHEMA" "0005_profile_cabinet_parity" \
  "0042_real_business_subscriptions" "$BACKEND_DIR"
'@

$PreflightOutput = Invoke-RemoteBash `
    -Script $PreflightScript `
    -FailureCode "STAGING_V9_PREFLIGHT_FAILED"
$PreflightText = $PreflightOutput -join "`n"
if ($PreflightText -notmatch "STAGING_V9_GUARD_OK") {
    throw "STAGING_V9_GUARD_CONFIRMATION_MISSING"
}
if ($PreflightText -notmatch [Regex]::Escape("SCHEMA=$ExpectedSchema")) {
    throw "STAGING_V9_SCHEMA_CONFIRMATION_MISMATCH"
}
if ($PreflightText -notmatch [Regex]::Escape("INITIAL_HEAD=$ExpectedInitialHead")) {
    throw "STAGING_V9_INITIAL_HEAD_CONFIRMATION_MISMATCH"
}
if ($PreflightText -notmatch [Regex]::Escape("FINAL_HEAD=$ExpectedFinalHead")) {
    throw "STAGING_V9_FINAL_HEAD_CONFIRMATION_MISMATCH"
}
...
ARCHIVE="/tmp/koprik-phase3c-v9-input/koprik-phase3c-source-final.tar.gz"
EXPECTED_ARCHIVE_SHA256="__EXPECTED_ARCHIVE_SHA256__"
EXPECTED_SCHEMA="0009_phase3c_real_subscriptions_v1"
EXPECTED_INITIAL_HEAD="0005_profile_cabinet_parity"
EXPECTED_FINAL_HEAD="0042_real_business_subscriptions"
NORMALIZATION_BATCH_SIZE="__NORMALIZATION_BATCH_SIZE__"
WORK="/tmp/koprik-phase3c-v9-$(date +%Y%m%d-%H%M%S)"
...
# Target bazaga birinchi yozuvdan oldingi read-only diagnostika. Demo egalar
# prune qilingan, qolgan Plus/Pro yozuvlar esa haqiqiy biznes va userga
# bog'langan bo'lishi shart.
python - "$SNAPSHOT_DB" <<'PY'
import sys

from app.legacy_migration.real_source_v9 import open_real_snapshot


source = open_real_snapshot(sys.argv[1])
try:
    paid_rows = source.execute(
        "SELECT bs.id "
        "FROM business_subscriptions AS bs "
        "JOIN businesses AS b ON b.id = bs.business_id "
        "JOIN users AS u ON u.id = b.user_id "
        "WHERE lower(trim(bs.plan_code)) IN ('plus','pro') "
        "AND lower(coalesce(u.login,'')) NOT LIKE 'demo_v1616_%' "
        "ORDER BY bs.id"
    ).fetchall()
    unsafe_rows = source.execute(
        "SELECT bs.id "
        "FROM business_subscriptions AS bs "
        "LEFT JOIN businesses AS b ON b.id = bs.business_id "
        "LEFT JOIN users AS u ON u.id = b.user_id "
        "WHERE lower(trim(bs.plan_code)) IN ('plus','pro') "
        "AND (b.id IS NULL OR u.id IS NULL "
        "OR lower(coalesce(u.login,'')) LIKE 'demo_v1616_%') "
        "ORDER BY bs.id"
    ).fetchall()
finally:
    source.close()
...
if not paid_rows:
    raise RuntimeError("real_business_subscription_source_rows_missing")
if unsafe_rows:
    raise RuntimeError(
        "real_business_subscription_source_owners_unsafe:"
        + ",".join(str(row[0]) for row in unsafe_rows)
    )
print(
    "REAL_BUSINESS_SUBSCRIPTIONS_SOURCE_OK "
    f"COUNT={len(paid_rows)} LEGACY_IDS={[int(row[0]) for row in paid_rows]}"
)
PY
...
try:
    EXPECTED_ROWS = {
        ...
        for row in source.execute(
            ...
            "WHERE lower(trim(plan_code)) IN ('plus','pro') "
            "ORDER BY id"
        ).fetchall()
    }
finally:
    source.close()
EXPECTED_LEGACY_IDS = set(EXPECTED_ROWS)
        ...
        async with database.session() as session:
                ...
                (
                    await session.scalars(
                        select(BusinessSubscription).where(
                            BusinessSubscription.legacy_source_id.is_not(None)
                        )
                    )
                ).all()
            )
            mapping_rows = (
                await session.execute(
                    select(LegacyIdMap.legacy_id, LegacyIdMap.target_id).where(
                        LegacyIdMap.entity_type == "business_account",
                        LegacyIdMap.target_id.is_not(None),
                    )
                )
            ).all()
            mappings = {
                int(legacy_id): int(target_id)
                for legacy_id, target_id in mapping_rows
            }
            ACTUAL_LEGACY_IDS = {
            ...
            if ACTUAL_LEGACY_IDS != EXPECTED_LEGACY_IDS:
                raise RuntimeError(
                    "real_business_subscription_legacy_ids_mismatch:"
                    f"expected={sorted(EXPECTED_LEGACY_IDS)}:"
                    f"actual={sorted(ACTUAL_LEGACY_IDS)}"
                )
            for row in subscriptions:
                source_row = EXPECTED_ROWS[int(row.legacy_source_id)]
                expected_account_id = mappings.get(int(source_row["business_id"]))
                actual = (
                ...
                expected = (
                    expected_account_id,
                    str(source_row["plan_code"]).strip().casefold(),
                    int(source_row["duration_months"]),
                    int(source_row["starts_at"]),
                    int(source_row["expires_at"]),
                    str(source_row["status"]).strip().casefold(),
                    bool(source_row["is_demo"]),
                    int(source_row["created_at"]),
                )
                if actual != expected:
                    raise RuntimeError(
                        "real_business_subscription_row_mismatch:"
                        f"legacy_id={row.legacy_source_id}"
                    )
            print(
                "REAL_BUSINESS_SUBSCRIPTIONS_OK "
                f"COUNT={len(ACTUAL_LEGACY_IDS)} "
                f"LEGACY_IDS={sorted(ACTUAL_LEGACY_IDS)}"
            )
    finally:
        await database.stop()
...
printf 'PHASE3C_V9_STAGING_COMPLETE RUN_ID=%s WORK=%s\n' "$RUN_ID" "$WORK"
'@
...
Invoke-RemoteBash `
    ...
    -FailureCode "STAGING_V9_MIGRATION_FAILED" | Out-Null

Write-Host "Phase 3C V9 fresh staging data migration finished."
