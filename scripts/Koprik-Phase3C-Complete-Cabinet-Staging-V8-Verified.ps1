...
$BaseScript = Join-Path $PSScriptRoot "Koprik-Phase3C-Complete-Cabinet-Staging-V8.ps1"
if (-not (Test-Path -LiteralPath $BaseScript -PathType Leaf)) {
    throw "V8_BASE_SCRIPT_NOT_FOUND"
}

Write-Host "SCRIPT_VERSION=8-verified"
...
function Invoke-RemoteBash {
    ...
    $InputPath = Join-Path $TempRoot "koprik-v8-verify-$Token.sh"
    $OutputPath = Join-Path $TempRoot "koprik-v8-verify-$Token.out"
    $ErrorPath = Join-Path $TempRoot "koprik-v8-verify-$Token.err"
...
fail() {
  printf 'PHASE3C_V8_VERIFY_ERROR=%s\n' "$1" >&2
  exit 1
}
...
WORK="$(
  find /tmp -maxdepth 1 -type d -name 'koprik-phase3c-v8-*' \
    -printf '%T@ %p\n' 2>/dev/null \
    | sort -nr \
    | head -n 1 \
    | cut -d' ' -f2-
)"
test -n "$WORK" || fail "v8_work_directory_not_found"
test -d "$WORK" || fail "v8_work_directory_not_found"
...
VERIFY_OUTPUT="$(
  python -m app.legacy_migration.profile_media_verify_v8 \
    --snapshot "$SNAPSHOT" \
    --run-id "$RUN_ID" \
    --environment staging
...
printf '%s\n' "$VERIFY_OUTPUT"
printf '%s\n' "$VERIFY_OUTPUT" \
  | grep -q '^PROFILE_MEDIA_V8_VERIFY_OK ' \
  || fail "profile_media_verify_confirmation_missing"

printf 'PHASE3C_V8_VERIFIED_COMPLETE RUN_ID=%s WORK=%s\n' "$RUN_ID" "$WORK"
'@

$VerifyOutput = Invoke-RemoteBash `
    -Script $VerifyScript `
    -FailureCode "STAGING_V8_PROFILE_MEDIA_GATE_FAILED"
$VerifyText = $VerifyOutput -join "`n"
if ($VerifyText -notmatch "PHASE3C_V8_VERIFIED_COMPLETE") {
    throw "STAGING_V8_VERIFIED_CONFIRMATION_MISSING"
}

Write-Host "Phase 3C V8 candidate + profile media gate finished."
