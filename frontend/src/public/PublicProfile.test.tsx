...
describe("PublicProfile", () => {
  ...
  it("renders the modular public business profile returned by its public id", async () => {
    const getPublicProfile = vi.fn().mockResolvedValue({
      kind: "business",
      public_id: "b_turon",
      name: "Turon savdo",
  ...
  it("checks the provider count before the login guard with exact modular copy", async () => {
    const user = userEvent.setup();
    const onBookQueue = vi.fn();
    const onNeedLogin = vi.fn();
  ...
  it("shows modular course metadata and opens enrollment only while admission is open", async () => {
    const user = userEvent.setup();
