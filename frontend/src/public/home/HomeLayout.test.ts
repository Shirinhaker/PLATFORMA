...
describe("modular mobile Home layout contract", () => {
  it("keeps the single-screen grid, story rail, map, and photo pin rules", () => {
    const css = readFileSync("src/public/public.css", "utf8");
  ...
  it("keeps the exact modular dark palette available to the Home shell", () => {
    const css = readFileSync("src/public/public.css", "utf8");
    const normalized = css.replace(/\s+/g, " ");
