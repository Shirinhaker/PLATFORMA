...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`documents-modular__status ${STATUS_CLASS[status] ?? ""}`}>
      {status}
    </span>
  );
}
...
function ScreenHeader({ title, onBack }: { title: string; onBack: () => void }) {
  return (
    <header className="documents-modular__header">
      <button type="button" className="documents-modular__back" onClick={onBack}>
        <span className="documents-modular__sr-only">Orqaga</span>
        ←
      </button>
      <h1>{title}</h1>
    </header>
...
export function Documents({
  ...
  const feedback = (
    <>
      {error ? <p className="documents-modular__error" role="alert">{error}</p> : null}
      {notice ? <p className="documents-modular__notice" role="status">{notice}</p> : null}
    </>
  );

  if (view === "profile") {
    return (
      <main className="documents-modular">
        <ScreenHeader title="Mening hujjatlarim" onBack={onBack} />
        <section className="documents-modular__form">
          <p className="documents-modular__info">
          ...
          {feedback}
          <button type="button" className="documents-modular__primary" disabled={busy} onClick={saveProfile}>
            Saqlash
          </button>
        </section>
      </main>
  ...
  if (view === "center") {
    ...
    return (
      <main className="documents-modular">
        ...
        <section className="documents-modular__menu">
          {cards.map((card) => (
  ...
  if (view === "counterparties") {
    return (
      <main className="documents-modular">
        ...
        <section className="documents-modular__total">
          <small>Kontragentlar</small><strong>{counterparties.length}</strong>
        </section>
        {canManageCounterparties ? (
          <button type="button" className="documents-modular__primary" onClick={() => openCounterparty()}>
            + Kontragent qo'shish
          </button>
        ) : null}
        {feedback}
        ...
        {!loading && !counterparties.length ? (
          <p className="documents-modular__empty">
            Hozircha kontragent yo'q. "+ Kontragent qo'shish" bilan qo'shing.
          </p>
        ) : null}
        <section className="documents-modular__list">
    ...
    return (
      <main className="documents-modular">
        ...
        <section className="documents-modular__form">
          ...
          {feedback}
          <button type="button" className="documents-modular__primary" disabled={busy} onClick={saveCounterparty}>Saqlash</button>
          {counterparty ? (
            <button type="button" className="documents-modular__soft" disabled={busy} onClick={removeCounterparty}>O'chirish</button>
          ) : null}
        </section>
      </main>
    );
    ...
    return (
      <main className="documents-modular">
        ...
        <section className="documents-modular__form">
          ...
          <div className="documents-modular__pair">
          ...
          <button type="button" className="documents-modular__soft" onClick={generateTemplate}>📄 Shablonni yuklash</button>
          <label>
            Hujjat matni
            <textarea
              rows={16}
          ...
          {feedback}
          <button type="button" className="documents-modular__primary" disabled={busy} onClick={saveNewDocument}>💾 Hujjatni saqlash</button>
          <button type="button" className="documents-modular__soft" onClick={() => copy(compose.body)}>📋 Nusxa olish</button>
        </section>
      </main>
    );
  }
  ...
  if (view === "list") {
    ...
    return (
      <main className="documents-modular">
        ...
        {direction !== "kiruvchi" ? (
          <button type="button" className="documents-modular__primary" onClick={() => openCompose(direction)}>
        ...
        <section className="documents-modular__list">
          {documents.map((row) => (
            <button type="button" key={row.id} onClick={() => openDocument(row)}>
              ...
              {direction !== "kiruvchi" && preview(row.body) ? <small className="documents-modular__preview">{preview(row.body)}...</small> : null}
            </button>
          ))}
        </section>
      </main>
  ...
  return (
    <main className="documents-modular">
      ...
      <section className="documents-modular__document-head">
      ...
      <textarea
        className="documents-modular__body"
        rows={16}
        readOnly={incoming}
      ...
      {selected.direction === "chiquvchi" ? (
        <section className="documents-modular__send">
      ...
      {!incoming ? (
        <>
          <button type="button" className="documents-modular__primary" disabled={busy} onClick={saveSelected}>💾 O'zgarishni saqlash</button>
          <button type="button" className="documents-modular__soft" onClick={() => copy(selected.body)}>📋 Nusxa olish</button>
          <button type="button" className="documents-modular__soft documents-modular__danger" disabled={busy} onClick={removeSelected}>🗑️ O'chirish</button>
        </>
      ) : (
        <>
          <button type="button" className="documents-modular__soft" onClick={() => copy(selected.body)}>📋 Nusxa olish</button>
          {selected.status === "kutilmoqda" ? (
            <section className="documents-modular__actions">
              <button type="button" className="documents-modular__primary" disabled={busy} onClick={() => respond("qabul")}>✅ Qabul qilish</button>
              <button type="button" className="documents-modular__soft documents-modular__danger" disabled={busy} onClick={() => respond("rad")}>❌ Rad etish</button>
            </section>
          ) : null}
        </>
      )}
