...
describe("modular savat state parity", () => {
  ...
  it("uses fractional steps only for modular fractional units and removes zero", () => {
...
describe("modular Savat screen parity", () => {
  ...
  it("asks with the exact modular confirmation before clearing a receipt", async () => {
    const user = userEvent.setup();
    render(<StatefulCart />);
  ...
  it("switches pickup and booking fields exactly like modular", async () => {
    const user = userEvent.setup();
    render(<StatefulCart />);
    await user.click(screen.getByRole("button", { name: "Buyurtma qilish" }));
