...
describe("modular jonli buyurtma kabineti", () => {
  it("mijoz mahsulot buyurtmalarini API dan olib, xizmatlarni alohida saqlaydi", async () => {
    ...
    rerender(<OrdersCabinet api={api} side="customer" category="service" onBack={vi.fn()} />);
  ...
  it("detailni ochganda o‘qilgan qiladi va chek, xarita, to‘lov hamda chatni ko‘rsatadi", async () => {
    ...
    render(<OrdersCabinet api={api} side="customer" category="product" onBack={vi.fn()} />);
  ...
  it("javob preview'i va chat rasmini modular ichki viewerida ko‘rsatadi", async () => {
    const user = userEvent.setup();
    const current = order();
    const api = apiFor([current]);
  ...
  it("pickup tayyor buyurtmani tasdiqdan keyin mijozga topshiradi", async () => {
    ...
    render(<OrdersCabinet api={api} side="provider" category="product" onBack={vi.fn()} />);
    await user.click(await screen.findByText("Buyurtma: Turon savdo"));
  ...
  it("biznes to‘lovni modular tasdiqlash oynasidan keyin tasdiqlaydi", async () => {
    const user = userEvent.setup();
  ...
  it("biznes qabul qilish, muammo ochish va topshirishni jonli endpointlarga yuboradi", async () => {
    ...
    const first = render(<OrdersCabinet api={api} side="provider" category="product" onBack={vi.fn()} />);
    await user.click(await screen.findByText("Buyurtma: Turon savdo"));
    ...
    render(<OrdersCabinet api={submittedApi} side="provider" category="product" onBack={vi.fn()} />);
    await user.click(await screen.findByText("Buyurtma: Turon savdo"));
  ...
  it("mijoz muammo yechimini va qabul qilinganini tasdiqlaydi", async () => {
    ...
    const first = render(<OrdersCabinet api={api} side="customer" category="product" onBack={vi.fn()} />);
    await user.click(await screen.findByText("Buyurtma: Turon savdo"));
    ...
    render(<OrdersCabinet api={deliveredApi} side="customer" category="product" onBack={vi.fn()} />);
    await user.click(await screen.findByText("Buyurtma: Turon savdo"));
