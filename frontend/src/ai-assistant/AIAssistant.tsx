...
export function AIAssistant({
  ...
  useEffect(() => {
    ...
    api
      .getAIChatHistory(30)
      .then((result) => active && setMessages(result.history))
      .catch(() => undefined)
    ...
    <section className="ai1656">
      ...
      <div className="ai1656__list" ref={listRef} aria-live="polite">
        ...
        {!loading && !messages.length ? (
          <div className="ai1656__empty">
            <span>🤖</span>
            <strong>AI yordamchi</strong>
            <p className="idesc">
              Savdo, ombor, qarz va buyurtmalar bo'yicha savol bering. Masalan: "Bugun
              qanday?"
            </p>
          </div>
        ) : null}
        {messages.map((message, index) => (
          <div
            ...
            className={`ai1656__row ai1656__row--${message.role}`}
          >
            <div>{message.text}</div>
          </div>
        ))}
        {sending ? (
          <div className="ai1656__row ai1656__row--assistant">
            <div className="ai1656__muted">yozmoqda...</div>
          </div>
        ) : null}
      </div>
      <div className="ai1656__chips">
        {CHIPS.map((chip) => (
          <button
            type="button"
            className="seg-b"
      ...
      <form
        className="ai1656__composer chat-compose"
        onSubmit={(event) => {
      ...
      <p className="ai1656__note idesc">
        AI ma'lumotlarni o'qiydi va tushuntiradi. O'zi o'zgartirish kiritmaydi.
      </p>
    </section>
  );
}
