...
function SpecialistMap({
  ...
  return (
    <>
      <div className="specialist-map-modular">
        <div ref={node} className="specialist-map-modular__canvas" />
        <div className="specialist-map-modular__pin">📍</div>
        <div className="specialist-map-modular__help">
          <span>Xaritani suring — metka markazda turadi</span>
        </div>
      </div>
      <div className="specialist-modular__muted specialist-modular__map-info">
        {latitude === null || longitude === null
          ? "Joy hali belgilanmagan"
          : `${latitude.toFixed(6)}, ${longitude.toFixed(6)}`}
      </div>
      {latitude === null || longitude === null ? (
        <div className="specialist-modular__warning">
          ⚠️ Qidiruv va xaritada ko‘rinish uchun joylashuvingizni xaritada belgilang.
        </div>
      ) : null}
      <button className="specialist-modular__soft" type="button" onClick={locate}>
...
export function Specialist({ api, onBack, onReviews }: Props) {
  ...
  async function uploadFile(file: File, purpose: Parameters<ApiClient["createUploadGrant"]>[0]["purpose"]) {
  ...
  async function addCredentials(files: FileList | null) {
    ...
    try {
      for (const file of Array.from(files)) {
        if (!IMAGE_TYPES.has(file.type)) throw new Error("Faqat JPG, PNG yoki WEBP rasm yuboring.");
        if (file.size < 1 || file.size > MAX_IMAGE_BYTES) throw new Error("Fayl hajmi 8 MB dan oshmasin.");
  ...
  if (!profile) {
    return <main className="specialist-modular"><p>Yuklanmoqda...</p></main>;
  }

  if (offerForm) {
    return (
      <main className="specialist-modular">
        <header className="specialist-modular__toolbar">
          <button type="button" onClick={() => setOfferForm(false)}>←</button>
          <h1>{offerId ? "Taklifni tahrirlash" : "Yangi taklif"}</h1>
        </header>
        <form className="specialist-modular__form" onSubmit={saveOffer}>
          ...
          />
          {offerImageUrl ? (
            <div className="specialist-modular__image-preview">
            ...
            <label className="specialist-modular__photo-add" htmlFor="specialist-offer-image">
              <span>📷</span> Rasm qo‘shish
            </label>
          )}
          <label>Turi</label>
          <div className="specialist-modular__chips">
            {(["service", "product"] as const).map((kind) => (
              ...
              >{kind === "service" ? "Xizmat" : "Mahsulot"}</button>
            ))}
          </div>
          <label>Nomi<input value={offer.name} placeholder="Masalan: Huquqiy maslahat" onChange={(event) => {
            const value = event.currentTarget.value;
            setOffer((current) => ({ ...current, name: value }));
          }} /></label>
          <label>Narxi<input value={offer.price_text} placeholder="Masalan: 100 000 so'm yoki kelishilgan" onChange={(event) => {
            const value = event.currentTarget.value;
            setOffer((current) => ({ ...current, price_text: value }));
          }} /></label>
          <label>Qisqa tavsif<textarea value={offer.note} placeholder="Xizmat yoki mahsulot haqida" onChange={(event) => {
            const value = event.currentTarget.value;
            setOffer((current) => ({ ...current, note: value }));
          }} /></label>
          {error ? <p className="specialist-modular__error" role="alert">{error}</p> : null}
          <button className="specialist-modular__primary" disabled={busy} type="submit">Saqlash</button>
          {offerId ? (
            <button
              className="specialist-modular__danger"
              disabled={busy}
              type="button"
              onClick={() => void remove(
                "Bu mahsulot/xizmat o'chirilsinmi?",
                () => api.deleteSpecialistOffer(offerId),
              ).then((deleted) => { if (deleted) setOfferForm(false); })}
            >O‘chirish</button>
          ) : null}
        </form>
      </main>
  ...
  return (
    <main className="specialist-modular">
      <header className="specialist-modular__toolbar">
        ...
        {onReviews ? (
          <button className="specialist-modular__reviews" type="button" onClick={onReviews}>
            💬 Mijoz fikrlari <b>{profile.review_count}</b>
          </button>
        ) : null}
      </header>

      <div className="specialist-modular__form">
        <label>Kasb / yo‘nalish<input value={profile.profession} placeholder="Masalan: Shifokor, advokat, santexnik" onChange={(event) => {
          const value = event.currentTarget.value;
          setProfile((current) => current && ({ ...current, profession: value }));
        }} /></label>
        <label>Qisqa tavsif<textarea value={profile.description} placeholder="Tajribangiz va qanday yordam bera olishingiz haqida qisqacha yozing" onChange={(event) => {
          const value = event.currentTarget.value;
          setProfile((current) => current && ({ ...current, description: value }));
        }} /></label>

        <section className="specialist-modular__section">
          <h2>Mutaxassislikni tasdiqlovchi hujjatlar</h2>
          <p>Diplom, sertifikat, litsenziya yoki guvohnoma rasmlarini joylang.</p>
          <div className="specialist-modular__rail">
            <label className="specialist-modular__add-card">
              <input type="file" accept="image/jpeg,image/png,image/webp" multiple hidden disabled={busy} onChange={(event) => void addCredentials(event.currentTarget.files)} />
              <span>+</span>Hujjat rasmi qo‘shish
            </label>
            {profile.credentials.map((item) => (
              <div className="specialist-modular__media-card" key={item.id}>
                <img alt="Hujjat" src={item.image_url} />
                <button type="button" onClick={() => void remove("Bu hujjat rasmi olib tashlansinmi?", () => api.deleteSpecialistCredential(item.id))}>×</button>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="specialist-modular__section">
          ...
          <div className="specialist-modular__rail">
            <button className="specialist-modular__add-card" type="button" onClick={() => editOffer()}>
              <span>+</span>Xizmat yoki mahsulot qo‘shish
            </button>
            {profile.offers.map((item) => (
              <button className="specialist-modular__offer-card" key={item.id} type="button" onClick={() => editOffer(item)}>
                <div className="specialist-modular__offer-image">{item.image_url ? <img alt="" src={item.image_url} /> : item.kind === "product" ? "📦" : "🧰"}</div>
        ...
        <section className="specialist-modular__section">
          ...
          <div className="specialist-modular__rail">
            <label className="specialist-modular__add-card">
              <input type="file" accept="image/jpeg,image/png,image/webp,video/mp4,video/webm,video/quicktime" multiple hidden disabled={busy} onChange={(event) => void addPortfolio(event.currentTarget.files)} />
              <span>+</span>Rasm yoki video qo‘shish
            </label>
            {profile.portfolio.map((item) => (
              <div className="specialist-modular__media-card" key={item.id}>
                {item.media_type === "video"
                  ? <><video muted playsInline preload="metadata" src={item.media_url} /><em>▶ VIDEO</em></>
                  : <img alt="Ish namunasi" src={item.media_url} />}
                <button type="button" onClick={() => void remove("Bu ish namunasi olib tashlansinmi?", () => api.deleteSpecialistPortfolio(item.id))}>×</button>
              </div>
            ))}
          </div>
        </section>

        <section className="specialist-modular__visibility">
          <h2>Ko‘rinishim</h2>
          <button className={profile.visible ? "on" : ""} type="button" onClick={() => setProfile((current) => current && ({ ...current, visible: true }))}><span>✅</span><div><b>Ko‘rinaman</b><p>Qidiruv va xaritada mutaxassis sifatida chiqaman.</p></div></button>
          <button className={!profile.visible ? "on" : ""} type="button" onClick={() => setProfile((current) => current && ({ ...current, visible: false }))}><span>🚫</span><div><b>Ko‘rinmayman</b><p>Profilim qidiruv va xaritada ko‘rinmaydi.</p></div></button>
        </section>

        <section>
          <h2>📍 Joylashuvingiz <small>(qidiruv va xaritada shu yerda chiqasiz)</small></h2>
          <SpecialistMap
            latitude={profile.latitude}
            longitude={profile.longitude}
            onChange={(latitude, longitude) => setProfile((current) => current && ({ ...current, latitude, longitude }))}
            onError={setError}
          />
        </section>

        {error ? <p className="specialist-modular__error" role="alert">{error}</p> : null}
        {saved ? <p className="specialist-modular__success" role="status">Saqlandi ✅</p> : null}
        <button
          className="specialist-modular__primary"
          disabled={busy}
          type="button"
          onClick={() => {
            setBusy(true);
