...
export function Notifications({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setNextCursor(notifications.next_cursor ?? null);
  ...
  async function loadOlder() {
    ...
    setBusy(true);
    try {
      const result = await api.getNotifications(nextCursor);
      setItems((current) => {
        const merged = new Map(result.items.map((row) => [row.id, row]));
  ...
  if (formOpen) {
    return (
      <main className="notifications-modular notifications-modular--form">
        <header className="notifications-modular__header">
        ...
        <fieldset>
          ...
          <div className="notifications-modular__price-grid">
            <input
              aria-label="Narx dan"
              inputMode="numeric"
        ...
        {error ? <p role="alert" className="notifications-modular__error">{error}</p> : null}
        <button type="button" className="notifications-modular__primary" disabled={busy} onClick={() => void saveFilter()}>
          Saqlash
        </button>
        <button type="button" className="notifications-modular__secondary" onClick={() => setFormOpen(false)}>
          Bekor qilish
        </button>
      </main>
    );
  ...
  return (
    <main className="notifications-modular">
      <header className="notifications-modular__header">
      ...
      <div className="notifications-modular__push-row">
      ...
      <div className="notifications-modular__hint">{pushStatusText(pushStatus)}</div>
      {error ? <p role="alert" className="notifications-modular__error">{error}</p> : null}
      {notice ? <p role="status" className="notifications-modular__notice">{notice}</p> : null}
      <div className="notifications-modular__section-head">
        <b>Buyurtma bildirishnomalari</b>
        <button type="button" disabled={busy || !items.length} onClick={() => void markAll()}>
          Barchasini o‘qish
        </button>
      </div>
      <div className="notifications-modular__list" aria-busy={loading}>
        {nextCursor ? (
          <button
            type="button"
            className="notifications-modular__secondary"
            disabled={busy}
            onClick={() => void loadOlder()}
          >
            Oldingi bildirishnomalarni yuklash
          </button>
        ) : null}
        {items.length ? items.map((notification) => (
          <button
            type="button"
            className={`notifications-modular__card${notification.is_read ? " is-read" : ""}`}
            key={notification.id}
            onClick={() => void markOne(notification)}
          >
            <span className="notifications-modular__icon">{notification.is_read ? "🔔" : "🟢"}</span>
            <span className="notifications-modular__copy">
          ...
          <div className="notifications-modular__empty">
            <h2>{loading ? "Yuklanmoqda..." : "Hozircha xabar yo‘q"}</h2>
            {!loading ? <p>Buyurtma yangiliklari shu yerda chiqadi.</p> : null}
          </div>
        )}
      </div>
      <div className="notifications-modular__divider" />
      <h2>E’lon filtrlari</h2>
      <p>Mos e’lon joylanganda Telegramingizga xabar keladi.</p>
      <button type="button" className="notifications-modular__primary" onClick={() => setFormOpen(true)}>
        ➕ Yangi filtr qo‘shish
      </button>
      <div className="notifications-modular__filters">
        {filters.length ? filters.map((filter) => {
          const category = CATEGORY_MAP.get(filter.cat) ?? { icon: "📦", label: filter.cat };
          return (
            <div className="notifications-modular__card" key={filter.id}>
              <span className="notifications-modular__icon">{category.icon}</span>
              <span className="notifications-modular__copy">
      ...
      {deleteFilter !== null ? (
        <div className="notifications-modular__modal" role="dialog" aria-modal="true">
          <div>
...
export function ActionNotifications({
  ...
  return (
    <div className="action-notification-modular" role="status" aria-live="polite">
      <button
        type="button"
        className="action-notification-modular__open"
        onClick={async () => {
      ...
      <button
        type="button"
        className="action-notification-modular__close"
        aria-label="Yopish"
        onClick={() => {
