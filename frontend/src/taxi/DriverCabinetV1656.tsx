  ...
  return (
    <main className="screen active taxi-driver-v1656" data-screen="taxidrv">
      ...
      <div className={`panel-card taxi-driver-v1656__balance${driver.balance < driver.commission ? " low" : ""}`}>
        ...
        {driver.balance < driver.commission ? <p className="taxi-v1656__error">⚠️ Balans yetarli emas — zakaz olish uchun to'ldiring.</p> : null}
      </div>
      ...
      {error ? <p role="alert" className="elon-hint taxi-v1656__error">{error}</p> : null}
      <div className="taxi-driver-v1656__orders-head"><h2>Kelayotgan zakazlar</h2><button className="sort-chip" type="button" onClick={() => void loadOrders()}>↻ Yangilash</button></div>
      <DriverOrders
        meterKm={meterKm}
        meterPrice={(() => {
...
function DriverOrders({
  ...
  if (orders.current) {
    ...
    return <div className="panel-card taxi-driver-v1656__current"><RideLines ride={ride} />{ride.status === "ongoing" && ride.ozim ? <div className="taxi-driver-v1656__meter">📟 Bosib o'tilgan: <b>{meterKm.toFixed(1)} km</b> · ~{money(meterPrice)} so'm</div> : null}<div className="list-sub">Mijoz: {ride.customer?.name || "—"}</div><div className="taxi-call-v1656__actions">{ride.customer?.phone ? <a className="btn btn-soft" href={`tel:${ride.customer.phone}`}>📞</a> : null}{action ? <button className="btn btn-primary" type="button" onClick={() => void onAdvance(ride, action.status)}>{action.label}</button> : null}</div>{ride.from_lat != null ? <><DriverRideMap ride={ride} /><a className="btn btn-soft btn-block" href={navigation} target="_blank" rel="noreferrer">🧭 Yo'lni ochish (Google Maps)</a></> : null}</div>;
  }
...
function RideLines({ ride }: { ride: TaxiRide }) {
  return <div className="taxi-driver-v1656__ride-lines"><div className="list-sub">🟢 {ride.from_addr || "Boshlanish"}</div><div className="list-sub">{ride.ozim ? "🗣 Manzilni og'zaki aytadi" : `🔴 ${ride.to_addr || "Manzil"}`}</div>{ride.dist_km ? <div className="list-sub">~{ride.dist_km.toFixed(1)} km{ride.dur_min ? ` · ~${ride.dur_min} daq` : ""}</div> : null}{ride.price ? <div className="list-sub taxi-call-v1656__price">💰 ~{money(ride.price)} so'm (naqd)</div> : ride.ozim ? <div className="list-sub">Narx masofa bo'yicha</div> : null}</div>;
}
