...
export type DriverCabinetApi = Partial<Pick<
  ApiClient,
  | "getTaxiDriver"
  | "getTaxiPricing"
  | "saveTaxiDriver"
...
function actionFor(ride: TaxiRide): { label: string; status: TaxiRideStatus } | null {
  if (ride.kind === "dostavka") {
    if (ride.status === "accepted") return { label: "📍 Sotuvchiga yetib keldim", status: "arrived_store" };
    if (ride.status === "arrived_store") return { label: "📦 Dostavkani oldim", status: "pickup_requested" };
    if (ride.status === "in_delivery") return { label: "📍 Buyurtmachiga yetib keldim", status: "arrived_customer" };
    if (ride.status === "arrived_customer") return { label: "✅ Buyurtmani topshirdim", status: "delivered_waiting_customer" };
    return null;
  }
  if (ride.status === "accepted") return { label: "🚗 Yetib keldim", status: "arrived" };
  if (ride.status === "arrived") return { label: "▶️ Safarni boshlash", status: "ongoing" };
...
export function DriverCabinet({ api, onBack }: Props) {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => { void loadOrders(); }, 4000);
  ...
  async function save() {
    ...
    if (["taxi", "both"].includes(service) && ![model, plate, color].every((value) => value.trim())) {
  ...
  async function accept(rideId: number) {
    ...
    try {
      ...
      setDriver((current) => current ? {
        ...current, balance: value.balance, available: false, busy: true,
      } : current);
      setMessage(`Zakaz qabul qilindi! Komissiya: ${money(value.commission)} so'm yechildi.`);
      await loadOrders();
    } catch (reason) {
      setError(errorText(reason));
      await loadOrders();
  ...
  if (!driver) {
    return <main className="profile-shell"><p>{error || "Yuklanmoqda..."}</p></main>;
  }

  if (editing) {
    const carRequired = ["taxi", "both"].includes(service);
    return (
      <main className="screen active taxi-driver-modular" data-screen="taxidrv">
        ...
        <div className="biz-desc taxi-driver-modular__intro">Ma'lumotlaringizni to'ldiring. Tasdiqlangach, yaqin zakazlar sizga ko'rinadi. To'lov mijoz bilan naqd.</div>
        <div className="taxi-driver-modular__account"><div className="cab-logo">👤</div><div><b>{driver.name}</b><div className="list-sub">Akkaunt ismingiz — shu ishlatiladi</div></div></div>
        ...
        <div className="field"><label>Nima bilan ishlaysiz</label><div className="sort-row taxi-driver-modular__service-row">
        ...
        <div className="elon-hint taxi-driver-modular__car-note">{carRequired ? "Taxi uchun mashina rusumi, raqami va rangi majburiy." : "Faqat dostavka — mashina ma'lumoti ixtiyoriy."}</div>
        {error ? <p role="alert" className="elon-hint taxi-modular__error">{error}</p> : null}
  ...
  return (
    <main className="screen active taxi-driver-modular" data-screen="taxidrv">
      ...
      <div className={`panel-card taxi-driver-modular__balance${driver.balance < driver.commission ? " low" : ""}`}>
        <div><small>💳 Balansingiz</small><strong>{money(driver.balance)} so'm</strong></div>
        <button className="sort-chip" type="button" onClick={() => setMessage("Balansni to'ldirish: firma hisobiga pul o'tkazing, admin tasdiqlagach balans qo'shiladi. Avtomatik to'lov tez orada ulanadi.")}>➕ To'ldirish</button>
        <p>Har qabul qilingan zakaz uchun {money(driver.commission)} so'm yechiladi.</p>
        {driver.balance < driver.commission ? <p className="taxi-modular__error">⚠️ Balans yetarli emas — zakaz olish uchun to'ldiring.</p> : null}
      </div>
      <div className="field"><label>Holatim</label>
        <button disabled={driver.busy} className={`vis-card${available ? " on" : ""}`} type="button" onClick={() => void availability(true)}><span className="v-ic">🟢</span><div><h5>Bo'shman</h5><p>{driver.busy ? "Joriy zakaz yakunlangach avtomatik yoqiladi." : "Yangi zakazlar menga ko'rinadi."}</p></div></button>
        <button disabled={driver.busy} className={`vis-card${!available ? " on" : ""}`} type="button" onClick={() => void availability(false)}><span className="v-ic">🔴</span><div><h5>Bandman</h5><p>{driver.busy ? "Zakaz qabul qilingani uchun avtomatik band." : "Zakazlar kelmaydi."}</p></div></button>
      </div>
      <button className="btn btn-soft btn-block" type="button" onClick={() => setEditing(true)}>✎ Ma'lumotlarni tahrirlash</button>
      {message ? <p role="status" className="elon-hint">{message}</p> : null}
      {error ? <p role="alert" className="elon-hint taxi-modular__error">{error}</p> : null}
      <div className="taxi-driver-modular__orders-head"><h2>Kelayotgan zakazlar</h2><button className="sort-chip" type="button" onClick={() => void loadOrders()}>↻ Yangilash</button></div>
      <DriverOrders
        meterKm={meterKm}
        meterPrice={(() => {
          ...
          if (!meterKm) return 0;
          return Math.round(Math.max(band.min, band.base + band.per_km * meterKm) / 500) * 500;
        })()}
        orders={orders}
        onAccept={accept}
        onAdvance={advance}
...
function DriverOrders({
  ...
  if (orders.current) {
    ...
    const navigation = ride.to_lat != null && ride.to_lng != null && !ride.ozim
    ...
    return <div className="panel-card taxi-driver-modular__current"><RideLines ride={ride} />{ride.status === "ongoing" && ride.ozim ? <div className="taxi-driver-modular__meter">📟 Bosib o'tilgan: <b>{meterKm.toFixed(1)} km</b> · ~{money(meterPrice)} so'm</div> : null}<div className="list-sub">Mijoz: {ride.customer?.name || "—"}</div><div className="taxi-call-modular__actions">{ride.customer?.phone ? <a className="btn btn-soft" href={`tel:${ride.customer.phone}`}>📞</a> : null}{action ? <button className="btn btn-primary" type="button" onClick={() => void onAdvance(ride, action.status)}>{action.label}</button> : null}</div>{ride.from_lat != null ? <><DriverRideMap ride={ride} /><a className="btn btn-soft btn-block" href={navigation} target="_blank" rel="noreferrer">🧭 Yo'lni ochish (Google Maps)</a></> : null}</div>;
  }
  if (!orders.available) return <div className="empty"><h3>Siz «Bandman»siz</h3><p>Zakaz olish uchun yuqorida «Bo'shman»ni tanlang.</p></div>;
  if (!orders.pending.length) return <div className="empty"><h3>Hozircha zakaz yo'q</h3><p>Yangi zakaz chiqsa, shu yerda darhol ko'rinadi.</p></div>;
  return <>{orders.pending.map((ride) => <div className="panel-card" key={ride.id}><b>{ride.kind === "dostavka" ? "📦 Dostavka" : "🚖 Taxi"}</b><RideLines ride={ride} /><div className="list-sub">Mijoz: {ride.customer_name || "—"}</div><button className="btn btn-primary btn-block" type="button" onClick={() => void onAccept(ride.id)}>Qabul qilish</button></div>)}</>;
}


function RideLines({ ride }: { ride: TaxiRide }) {
  return <div className="taxi-driver-modular__ride-lines"><div className="list-sub">🟢 {ride.from_addr || "Boshlanish"}</div><div className="list-sub">{ride.ozim ? "🗣 Manzilni og'zaki aytadi" : `🔴 ${ride.to_addr || "Manzil"}`}</div>{ride.dist_km ? <div className="list-sub">~{ride.dist_km.toFixed(1)} km{ride.dur_min ? ` · ~${ride.dur_min} daq` : ""}</div> : null}{ride.price ? <div className="list-sub taxi-call-modular__price">💰 ~{money(ride.price)} so'm (naqd)</div> : ride.ozim ? <div className="list-sub">Narx masofa bo'yicha</div> : null}</div>;
}
...
function DriverRideMap({ ride }: { ride: TaxiRide }) {
  ...
  useEffect(() => {
    if (!host.current || ride.from_lat == null || ride.from_lng == null) return undefined;
    let disposed = false;
    ...
    void import("leaflet").then(async (module) => {
      ...
      instance = L.map(host.current, { attributionControl: false, zoomControl: true })
      ...
      if (!ride.ozim && ride.to_lat != null && ride.to_lng != null) {
        ...
        instance.fitBounds([
          [ride.from_lat!, ride.from_lng!], [ride.to_lat, ride.to_lng],
        ], { padding: [45, 45] });
        ...
        const payload = await fetch(url, { signal: controller.signal })
          ...
          .catch(() => null) as { routes?: Array<{ geometry: GeoJSON.GeoJsonObject }> } | null;
  ...
  return <div className="taxi-driver-modular__map" ref={host} />;
}
