...
function ProfileRow({
  ...
  return (
    <button
      type="button"
      className="elon-item follow-list-modular__row"
      aria-label={`${row.name} profilini ochish`}
      onClick={onOpen}
    >
      <span
        className="li-thumb follow-list-modular__avatar"
        style={{ background: business
          ? "var(--primary-tint)"
          : "var(--amber-tint)" }}
      >
        {row.image_url ? (
...
export function FollowLists({
  ...
  return (
    <main className="follow-list-modular">
      <header className="follow-list-modular__header">
        <h1>{title}</h1>
        <button type="button" onClick={onBack}>Orqaga</button>
      </header>
      {error ? (
        <div className="follow-list-modular__error" role="alert">
          <p>{error}</p>
          <button type="button" onClick={() => void load()}>Qayta urinish</button>
        </div>
      ) : loading ? (
        <p className="follow-list-modular__loading">Yuklanmoqda…</p>
      ) : items.length ? (
        ...
        <div className="empty follow-list-modular__empty">
