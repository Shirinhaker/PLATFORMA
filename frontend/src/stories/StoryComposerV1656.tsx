...
function videoDuration(file: File): Promise<number> {
  return new Promise((resolve, reject) => {
    ...
    const timeout = window.setTimeout(
      () => finish(new Error("Video davomiyligi aniqlanmadi.")),
      8000,
    );
    let complete = false;
...
export function StoryComposerV1656({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(
        reasonValue instanceof Error ? reasonValue.message : "Istoriya yaratilmadi.",
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <div
      aria-label="Istoriya yaratish"
      aria-modal="true"
      className="story-composer-v1656"
      role="dialog"
    >
      <form onSubmit={(event) => void submit(event)}>
        <header>
          <strong>Yangi istoriya</strong>
          <button aria-label="Yopish" type="button" onClick={onClose}>
            ×
          </button>
        </header>
        <label>
        ...
        <small>
          Rasm 10 MB gacha. Video 100 MB gacha. Video 60 soniyadan oshmasin.
        </small>
        <label>
          Izoh
          <textarea
