...
function EducationShell({
  ...
  return (
    <main className="education-management-modular">
      <header className="education-management-modular__heading">
...
function Empty({ children }: { children: ReactNode }) {
  return <div className="education-management-modular__empty">{children}</div>;
}
...
function ScheduleView({ api, onBack }: { api: EducationManagementApi; onBack: () => void }) {
  ...
  return (
    <EducationShell title="Dars jadvali" caption="Haftalik guruh va o'qituvchilar jadvali" onBack={onBack}>
      ...
      <section className="education-management-modular__filters two">
      ...
      {loading ? <Empty>Jadval yuklanmoqda…</Empty> : DAYS.map((day) => {
        ...
        return (
          <section className="education-management-modular__day" key={day.key}>
            ...
            {lessons.length ? lessons.map((group) => (
              <article className="education-management-modular__lesson" key={group.id}>
            ...
            )) : <p className="education-management-modular__muted">Dars belgilanmagan</p>}
          </section>
        );
      })}
    </EducationShell>
...
function AttendanceView({ api, onBack }: { api: EducationManagementApi; onBack: () => void }) {
  ...
  return (
    <EducationShell title="Davomat" caption="O'quvchilarning darsga qatnashuvi" onBack={onBack}>
      <section className="education-management-modular__intro"><b>Davomatni belgilash</b><p>Guruh va sanani tanlang. Avval saqlangan davomat avtomatik ochiladi.</p></section>
      <ErrorBox value={error} />
      {notice ? <p className="education-management-modular__success">{notice}</p> : null}
      <section className="education-management-modular__filters two">
      ...
      <div className="education-management-modular__actions two">
      ...
      {students.length ? <p className="education-management-modular__summary-line">Jami: {students.length} · Keldi: {counts.present} · Kechikdi: {counts.late} · Sababli: {counts.excused} · Sababsiz: {counts.absent}</p> : null}
      {loaded && !students.length ? <Empty><h3>O'quvchilar yo'q</h3><p>Tanlangan guruhga avval o'quvchilarni biriktiring.</p></Empty> : null}
      <section className="education-management-modular__cards">
        {students.map((student) => (
          <article key={student.student_id}>
            ...
            <div className="education-management-modular__attendance-buttons">
      ...
      {students.length ? <button className="education-management-modular__primary" type="button" disabled={busy} onClick={() => void save()}>Davomatni saqlash</button> : null}
    </EducationShell>
  );
}
...
function PaymentModal({
  ...
  return (
    <div className="education-management-modular__modal-back">
      <section className="education-management-modular__modal" role="dialog" aria-modal="true" aria-label="O'quvchi to'lovi">
        ...
        <div className="education-management-modular__modal-actions"><button type="button" disabled={busy} onClick={onClose}>Bekor qilish</button><button type="button" disabled={busy} onClick={() => onSave(numeric(amount), payType, note)}>To'lovni qabul qilish</button></div>
      </section>
    </div>
  );
}
...
function PaymentControlPane({
  ...
  return (
    <>
      <ErrorBox value={error} />
      <nav className="education-management-modular__tabs compact">
      ...
      <section className="education-management-modular__filters two"><input type="search" placeholder="Ism yoki telefon..." value={search} onChange={(event) => setSearch(event.target.value)} /><GroupSelect groups={groups} value={groupId} onChange={setGroupId} all /></section>
      <section className="education-management-modular__totals three"><span>Muddati o'tgan<b className="negative">{data.summary.overdue}</b></span><span>Bugun<b className="warning">{data.summary.due_today}</b></span><span>Jami qarz<b>{money(data.summary.total_debt)}</b></span></section>
      ...
      <section className="education-management-modular__cards">
        {rows.map((student) => {
          ...
          return <article key={student.id} className="collapsible"><button type="button" className="education-management-modular__card-toggle" aria-expanded={open} onClick={() => setExpanded(open ? 0 : student.id)}><b>{student.full_name}</b><span>›</span></button>{open ? <div className="education-management-modular__details"><p>Holat: <em className={student.status}>{statusLabel}</em><br />Guruh: {student.group_name || "Guruhsiz"}<br />To'lov turi: {student.billing_type === "attendance" ? "Dars paketi" : "Oylik"}<br />{student.billing_type === "attendance" ? `${student.lessons_done} dars hisoblandi · ${student.lessons_remaining} dars qoldi` : `Keyingi muddat: ${student.next_due || "—"}`}<br />Qarz: {money(student.debt)}</p>{student.debt ? <button className="education-management-modular__primary" type="button" onClick={() => setTarget({ student_id: student.id, full_name: student.full_name, payment_month: student.payment_month, debt: student.payable_now || student.debt })}>To'lov olish</button> : null}</div> : null}</article>;
        })}
      </section>
...
function PaymentHistoryPane({
  ...
  return (
    <>
      <ErrorBox value={error} />
      <section className="education-management-modular__filters two"><input type="month" value={month} onChange={(event) => setMonth(event.target.value)} /><GroupSelect groups={groups} value={groupId} onChange={setGroupId} all /></section>
      <section className="education-management-modular__totals three"><span>Hisoblandi<b>{money(totals.expected)}</b></span><span>To'landi<b className="positive">{money(totals.paid)}</b></span><span>Qarz<b className="negative">{money(totals.debt)}</b></span></section>
      {loading ? <Empty>Yuklanmoqda…</Empty> : null}
      <section className="education-management-modular__cards">
        {data.students.map((student: EducationPaymentStudent) => {
          ...
          return <article key={student.student_id} className="collapsible"><button type="button" className="education-management-modular__card-toggle" onClick={() => setExpanded(open ? 0 : student.student_id)}><b>{student.full_name}</b><span>›</span></button>{open ? <div className="education-management-modular__details"><p>Guruh: {student.group_name || "Guruhsiz"}<br />Holat: {status}<br />Hisoblandi: {money(student.expected)}<br />To'landi: {money(student.paid)}<br />Qoldi: {money(student.debt)}{student.billing_type === "attendance" ? <><br />Darslar: {student.chargeable_lessons} × {money(student.per_lesson_price)}</> : null}</p>{student.debt ? <button className="education-management-modular__primary" type="button" onClick={() => setTarget({ student_id: student.student_id, full_name: student.full_name, payment_month: month, debt: student.debt })}>To'lov qabul qilish</button> : null}</div> : null}</article>;
        })}
      </section>
      <h2 className="education-management-modular__section-title">To'lovlar tarixi</h2>
      <section className="education-management-modular__history">{data.history.map((payment) => <article key={payment.id} className={payment.voided_at ? "voided" : ""}><div><b>{payment.full_name}</b><small>{money(payment.amount)} · {payment.pay_type === "karta" ? "Karta" : "Naqd"}{payment.note ? ` · ${payment.note}` : ""}{payment.voided_at ? ` · Bekor qilingan: ${payment.void_reason || "Sabab ko'rsatilmagan"}` : ""}</small></div>{canVoid && !payment.voided_at ? <button type="button" disabled={busy} onClick={() => void voidPayment(payment.id)}>Bekor qilish</button> : null}</article>)}</section>
      {!data.history.length ? <p className="education-management-modular__muted">Bu oy uchun to'lovlar yo'q.</p> : null}
      {target ? <PaymentModal target={target} busy={busy} onClose={() => setTarget(null)} onSave={(amount, payType, note) => void pay(amount, payType, note)} /> : null}
    </>
  );
}
...
function TeachersView({ api, onBack }: { api: EducationManagementApi; onBack: () => void }) {
  ...
  return <EducationShell title="O'qituvchilar" caption="O'qituvchi, fan va ish haqi" onBack={onBack}><ErrorBox value={error} /><section className="education-management-modular__toolbar"><div><b>O'qituvchilar</b><small>{teachers.length} nafar faol o'qituvchi</small></div><button type="button" onClick={() => open()}>+ O'qituvchi</button></section><input className="education-management-modular__search" type="search" placeholder="Ism yoki fan bo'yicha qidirish..." value={search} onChange={(event) => setSearch(event.target.value)} />{loading ? <Empty>Yuklanmoqda…</Empty> : null}<section className="education-management-modular__cards">{rows.map((teacher) => <button className="education-management-modular__teacher" type="button" key={teacher.id} onClick={() => open(teacher)}><span>🧑‍🏫</span><div><b>{teacher.full_name}</b><small>{teacher.specialty || "Mutaxassislik belgilanmagan"}</small><em>{teacher.phone ? `📞 ${teacher.phone} · ` : ""}👥 {teacher.group_count} guruh{teacher.salary_amount ? ` · 💰 ${money(teacher.salary_amount)} ${teacher.salary_type === "per_lesson" ? "/ dars" : "/ oy"}` : ""}</em></div><i>›</i></button>)}</section>{!loading && !rows.length ? <Empty><h3>O'qituvchi topilmadi</h3><p>Yangi o'qituvchi qo'shing yoki qidiruvni o'zgartiring.</p></Empty> : null}{editing !== undefined ? <div className="education-management-modular__modal-back"><section className="education-management-modular__modal" role="dialog" aria-modal="true"><h2>{editing ? "O'qituvchini tahrirlash" : "Yangi o'qituvchi"}</h2><label>Ism-familiya<input value={form.full_name} maxLength={120} onChange={(event) => setForm({ ...form, full_name: event.target.value })} /></label><label>Telefon raqami<input value={form.phone} maxLength={30} onChange={(event) => setForm({ ...form, phone: event.target.value })} /></label><label>Fan yoki mutaxassisligi<input value={form.specialty} maxLength={120} onChange={(event) => setForm({ ...form, specialty: event.target.value })} /></label><label>Ishga kirgan sana<input type="date" value={form.hired_date} onChange={(event) => setForm({ ...form, hired_date: event.target.value })} /></label><label>Ish haqi turi<select value={form.salary_type} onChange={(event) => setForm({ ...form, salary_type: event.target.value as "monthly" | "per_lesson" })}><option value="monthly">Oylik maosh</option><option value="per_lesson">Har bir dars uchun</option></select></label><label>Ish haqi summasi<input inputMode="numeric" value={salary} onChange={(event) => setSalary(event.target.value)} /></label><label>Izoh<textarea value={form.note} maxLength={500} onChange={(event) => setForm({ ...form, note: event.target.value })} /></label><div className="education-management-modular__modal-actions">{editing ? <button type="button" className="danger" disabled={busy} onClick={() => void remove()}>Ro'yxatdan chiqarish</button> : null}<button type="button" disabled={busy} onClick={() => setEditing(undefined)}>Bekor qilish</button><button type="button" disabled={busy} onClick={() => void save()}>Saqlash</button></div></section></div> : null}</EducationShell>;
}
...
function PayrollModal({ target, month, busy, onClose, onSave }: { target: EducationPayrollTeacher; month: string; busy: boolean; onClose: () => void; onSave: (amount: number, payType: "naqd" | "karta", note: string) => void }) {
  ...
  const [payType, setPayType] = useState<"naqd" | "karta">("naqd");
  const [note, setNote] = useState("");
  return <div className="education-management-modular__modal-back"><section className="education-management-modular__modal" role="dialog" aria-modal="true"><h2>{target.full_name}</h2><p>Hisoblandi: {money(target.expected)} · To'landi: {money(target.paid)} · Qoldi: {money(target.debt)}</p><label>Oy<input type="month" readOnly value={month} /></label><label>To'lov summasi<input inputMode="numeric" value={amount} onChange={(event) => setAmount(event.target.value)} /></label><label>To'lov turi<select value={payType} onChange={(event) => setPayType(event.target.value as "naqd" | "karta")}><option value="naqd">Naqd</option><option value="karta">Karta</option></select></label><label>Izoh<input maxLength={200} value={note} onChange={(event) => setNote(event.target.value)} /></label><div className="education-management-modular__modal-actions"><button type="button" disabled={busy} onClick={onClose}>Bekor qilish</button><button type="button" disabled={busy} onClick={() => onSave(numeric(amount), payType, note)}>Maoshni to'lash</button></div></section></div>;
}
...
function PayrollView({ api, onBack }: { api: EducationManagementApi; onBack: () => void }) {
  ...
  return <EducationShell title="O'qituvchi maoshi" caption="Oy bo'yicha hisoblangan va to'langan maoshlar" onBack={onBack}><ErrorBox value={error} /><input className="education-management-modular__month" type="month" value={month} onChange={(event) => setMonth(event.target.value)} /><section className="education-management-modular__totals three"><span>Hisoblandi<b>{money(totals.expected)}</b></span><span>To'landi<b className="positive">{money(totals.paid)}</b></span><span>Qoldi<b className="negative">{money(totals.debt)}</b></span></section>{loading ? <Empty>Yuklanmoqda…</Empty> : null}<section className="education-management-modular__cards">{data.teachers.map((teacher) => <article key={teacher.id}><header><div><b>{teacher.full_name}</b><small>{teacher.salary_type === "per_lesson" ? `${teacher.lesson_count} dars × ${money(teacher.salary_amount)}` : `Oylik ${money(teacher.salary_amount)}`}</small></div><em>{teacher.debt > 0 ? `Qoldi ${money(teacher.debt)}` : teacher.expected ? "To'langan" : "Hisob yo'q"}</em></header><p>Hisoblandi: {money(teacher.expected)} · To'landi: {money(teacher.paid)}</p>{teacher.debt > 0 ? <button className="education-management-modular__primary" type="button" onClick={() => setTarget(teacher)}>Maosh to'lash</button> : null}</article>)}</section>{!loading && !data.teachers.length ? <Empty><h3>O'qituvchilar yo'q</h3></Empty> : null}<h2 className="education-management-modular__section-title">Maosh to'lovlari tarixi</h2><section className="education-management-modular__history">{data.history.map((payment) => <article key={payment.id}><div><b>{payment.full_name}</b><small>{money(payment.amount)} · {payment.pay_type === "karta" ? "Karta" : "Naqd"}{payment.note ? ` · ${payment.note}` : ""}</small></div><button type="button" disabled={busy} onClick={() => void remove(payment.id)}>O'chirish</button></article>)}</section>{!data.history.length ? <p className="education-management-modular__muted">Bu oyda maosh to'lovi yo'q.</p> : null}{target ? <PayrollModal target={target} month={month} busy={busy} onClose={() => setTarget(null)} onSave={(amount, payType, note) => void pay(amount, payType, note)} /> : null}</EducationShell>;
}

export function EducationManagement({ api, view, onBack, canVoidPayments = true }: { api: EducationManagementApi; view: EducationManagementView; onBack: () => void; canVoidPayments?: boolean }) {
  if (view === "education-groups") return <EducationGroups api={api} onBack={onBack} />;
  if (view === "education-students") return <EducationStudents api={api} onBack={onBack} />;
