...
export function DebtLedger({
  ...
  if (detail) {
    return (
      <main className="debt-modular">
        <header className="debt-modular__heading">
          <button type="button" onClick={() => { setDetail(null); void load(); }}>← Qarz daftari</button>
          <div><h1>{detail.name}</h1><p>{detail.phone}</p></div>
        </header>
        {error ? <p className="debt-modular__error" role="alert">{error}</p> : null}
        <section className="debt-modular__summary">
        ...
        <div className="debt-modular__section-title">
          <h2>Amaliyotlar tarixi</h2><span>{detail.tx.length} ta</span>
        </div>
        <section className="debt-modular__transactions">
  ...
  return (
    <main className="debt-modular">
      ...
      {loading ? <p className="debt-modular__empty">Yuklanmoqda…</p> : null}
      {!loading && !rows.length ? (
        <div className="debt-modular__empty"><h3>Hozircha qarzdor yo‘q</h3><p>Pastdagi tugma orqali qo‘shing.</p></div>
      ) : null}
      <section className="debt-modular__list">
        {rows.map((debtor) => (
          <button type="button" key={debtor.id} disabled={busy} onClick={() => openDebtor(debtor.id)}>
            <span className="debt-modular__avatar">{initials(debtor.name) || "?"}</span>
      ...
      <button type="button" className="debt-modular__add" disabled={busy} onClick={() => openForm("debtor")}>
        + Yangi qarzdor
      </button>
      {form === "debtor" ? (
        <div className="debt-modular__modal-back" role="presentation">
          <section className="debt-modular__modal" role="dialog" aria-modal="true" aria-label="Yangi qarzdor">
            <h2>Yangi qarzdor</h2>
            {error ? <p className="debt-modular__error" role="alert">{error}</p> : null}
            <div className="debt-modular__modal-fields">
            ...
            <div className="debt-modular__modal-actions">
...
function TransactionModal({
  ...
  return (
    <div className="debt-modular__modal-back" role="presentation">
      <section className="debt-modular__modal" role="dialog" aria-modal="true" aria-label={label}>
        <h2>{label}</h2>
        {error ? <p className="debt-modular__error" role="alert">{error}</p> : null}
        <div className="debt-modular__modal-fields">
