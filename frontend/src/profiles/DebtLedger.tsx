...
import type { DebtTransactionCreate, Debtor, DebtorDetail } from "../api/types";
    ...
    return (
      <main className="debt-v1656">
        ...
        <section className="debt-v1656__summary">
          ...
          <div>
            <button type="button" disabled={busy} onClick={() => openForm("payment")}>
              − To‘lov
            </button>
            <button type="button" disabled={busy} onClick={() => openForm("debt")}>
              + Qarz
            </button>
          </div>
        </section>
        <div className="debt-v1656__section-title">
          <h2>Amaliyotlar tarixi</h2>
          <span>{detail.tx.length} ta</span>
        </div>
  ...
  return (
    <main className="debt-v1656">
      <header className="debt-v1656__heading">
        ...
        <div>
          <h1>Qarz daftari</h1>
          <p>Mijozlar qarzlarini yuritish</p>
        </div>
      </header>
      {error ? (
        <p className="debt-v1656__error" role="alert">
      ...
      <section className="debt-v1656__summary">
        ...
        <p>
          <b>{rows.length}</b> ta qarzdor · <b>{summary.withDebt}</b> tasida qarz bor
        </p>
      </section>
      {loading ? <p className="debt-v1656__empty">Yuklanmoqda…</p> : null}
      {!loading && !rows.length ? (
        <div className="debt-v1656__empty">
          <h3>Hozircha qarzdor yo‘q</h3>
          <p>Pastdagi tugma orqali qo‘shing.</p>
        </div>
      ) : null}
      ...
      <button
        type="button"
        className="debt-v1656__add"
        disabled={busy}
        onClick={() => openForm("debtor")}
      >
        + Yangi qarzdor
      </button>
      {form === "debtor" ? (
        <div className="debt-v1656__modal-back" role="presentation">
          ...
          >
            ...
            <div className="debt-v1656__modal-fields">
              ...
              <label>
                Boshlang‘ich qarz (faqat raqam, ixtiyoriy)
                <input
                  inputMode="numeric"
                  value={amount}
            ...
            <div className="debt-v1656__modal-actions">
              ...
              <button type="button" disabled={busy} onClick={saveDebtor}>
                Qo‘shish
              </button>
            </div>
          </section>
