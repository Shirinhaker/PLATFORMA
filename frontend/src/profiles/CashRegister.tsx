...
function ReceiptCard({
  ...
  return (
    <article className="cash-modular__receipt">
      <header>
      ...
      <footer>
        ...
        {receipt.can_change_payment ? (
          <span className="cash-modular__payment-actions">
            <button
              type="button"
...
export function CashRegister({
  ...
  if (screen === "create") {
    ...
    return (
      <main className="cash-modular">
        <header className="cash-modular__heading">
          <button type="button" onClick={() => setScreen("list")}>← Kassa</button>
          <div><h1>Savdo yozish</h1><p>Bitta chekda bir nechta mahsulot</p></div>
        </header>
        {error ? <p className="cash-modular__error" role="alert">{error}</p> : null}
        <section className="cash-modular__form-card">
          <label>Mahsulot qidirish
            <input value={search} onChange={(event) => setSearch(event.target.value)} />
          </label>
          <div className="cash-modular__catalog">
          ...
          <button type="button" className="cash-modular__custom" onClick={addCustomItem}>
            + Boshqa mahsulot
          </button>
        </section>
        <section className="cash-modular__form-card">
          ...
          {!draft.length ? <p>Chek bo‘sh — mahsulot tanlang.</p> : (
            <div className="cash-modular__draft-lines">
              {draft.map((line) => (
          ...
          <div className="cash-modular__fields">
            ...
            {payType === "qarz" ? (
              <label>Qarzdor<div className="cash-modular__debtor-field">
            ...
            <label className="cash-modular__wide">Izoh<textarea maxLength={200} value={note}
              onChange={(event) => setNote(event.target.value)} /></label>
          </div>
          <div className="cash-modular__save">
  ...
  return (
    <main className="cash-modular">
      <header className="cash-modular__heading">
        ...
        <button type="button" className="cash-modular__add" disabled={busy} onClick={openCreate}>
          + Savdo yozish
        </button>
      </header>
      {error ? <p className="cash-modular__error" role="alert">{error}</p> : null}
      <section className="cash-modular__summary">
        <div className="cash-modular__days">
      ...
      {loading ? <div className="cash-modular__empty">Yuklanmoqda…</div> : null}
      {!loading && !register.receipts.length ? (
        <div className="cash-modular__empty">Bu kunda savdo yo‘q.</div>
      ) : null}
      {!loading ? (
        <section className="cash-modular__receipts">
