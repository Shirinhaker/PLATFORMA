...
export function Expenses({
  ...
  return (
    <main className="expenses-modular">
      <header className="expenses-modular__heading">
        <button type="button" onClick={onBack}>← Kabinetga qaytish</button>
        <div><h1>Xarajatlar</h1><p>Kunlik xarajatlar hisobi</p></div>
      </header>
      {error ? <p className="expenses-modular__error" role="alert">{error}</p> : null}
      <section className="expenses-modular__summary">
      ...
      <button
        type="button"
        className="expenses-modular__add"
        disabled={busy}
        onClick={() => void openForm()}
      >+ Xarajat yozish</button>
      <nav className="expenses-modular__days" aria-label="Xarajat sanasi">
      ...
      {loading ? <p className="expenses-modular__empty">Yuklanmoqda…</p> : null}
      {!loading && !data.expenses.length ? (
        <p className="expenses-modular__empty">Bu kunda xarajat yo‘q.</p>
      ) : null}
      <section className="expenses-modular__list">
        {data.expenses.map((expense) => {
          const fromStock = expense.source === "stock";
          return (
            <article key={expense.id}>
              <div className="expenses-modular__row">
                <b>{expense.category}{expense.note ? ` — ${expense.note}` : ""}</b>
                <strong>−{money(expense.amount)}</strong>
              </div>
              <div className="expenses-modular__meta">
      ...
      {formOpen ? (
        <div className="expenses-modular__modal-back" role="presentation">
          <section
            className="expenses-modular__modal"
            role="dialog"
            aria-modal="true"
          ...
          >
            ...
            <div className="expenses-modular__modal-actions">
