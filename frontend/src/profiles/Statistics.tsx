...
function StatisticsBars({
  ...
  return (
    <svg
      className="statistics-modular__chart"
    ...
    >
      {hasNegative ? (
        <line
          ...
          y2={zeroY}
          className="statistics-modular__zero"
        />
      ) : null}
        ...
        return (
          <g key={`${item.label}-${index}`}>
            <rect
              ...
              height={barHeight}
              rx="2"
              className={`statistics-modular__bar statistics-modular__bar--${metric}${value < 0 ? " statistics-modular__bar--negative" : ""}`}
            >
...
function ProgressRow({
  ...
  return (
    <div className="statistics-modular__progress-row">
...
export function Statistics({
  ...
  return (
    <main className="statistics-modular">
      <header className="statistics-modular__heading">
        <button type="button" onClick={onBack}>← Kabinetga qaytish</button>
        <div><h1>Statistika</h1><p>Tushum, xarajat, foyda va tovarlar</p></div>
      </header>

      <nav className="statistics-modular__periods" aria-label="Statistika davri">
        {PERIODS.map((option) => (
          <button
            type="button"
            key={option.key}
      ...
      <nav className="statistics-modular__navigation" aria-label="Davr navigatsiyasi">
        <button
          type="button"
      ...
      {error ? <p className="statistics-modular__error" role="alert">{error}</p> : null}
      {loading ? <p className="statistics-modular__empty">Yuklanmoqda…</p> : null}

      {!loading ? (
        <>
          <section className="statistics-modular__financial" aria-label="Moliyaviy natijalar">
            ...
            <article className="statistics-modular__profit">
          ...
          <section className="statistics-modular__panel">
            <nav className="statistics-modular__metrics" aria-label="Grafik ko‘rsatkichi">
            ...
            {data.trend.length ? <StatisticsBars items={data.trend} metric={metric} /> : <p className="statistics-modular__empty">Ma’lumot yo‘q</p>}
          </section>
          ...
          {data.top_products.length ? (
            <section className="statistics-modular__panel statistics-modular__products">
          ...
          {data.low_stock.length ? (
            <section className="statistics-modular__panel statistics-modular__stock">
          ...
          {data.waiters.length ? (
            <section className="statistics-modular__panel statistics-modular__employees">
              <h2>🧑‍🍳 Ofitsiantlar</h2>
