...
describe("StatisticsV1656", () => {
  it("renders the complete v1656 financial, source and employee parity", async () => {
    ...
    expect(screen.getByText(/Sof foyda/).closest("article")).toHaveTextContent(
      "855 000 so'm",
    );
    expect(screen.getByText(/Qarzdan qaytgan pul/)).toHaveTextContent("200 000");
    ...
    expect(
      within(products as HTMLElement)
        .getByText("Olma")
        .closest("article"),
  ...
  it("changes period, metric and date without reloading unrelated resources", async () => {
    ...
    expect(screen.getByRole("img", { name: "Xarajat grafigi" })).toBeInTheDocument();
    ...
    await waitFor(() =>
      expect(api.getStatistics).toHaveBeenCalledWith("kun", "2026-07-01"),
    );
  });
  ...
  it("does not let an older delayed response overwrite the selected period", async () => {
    ...
    const month = new Promise<StatisticsReport>((resolve) => {
    ...
    await waitFor(() =>
      expect(screen.queryByText("Kechikkan oy")).not.toBeInTheDocument(),
    );
  ...
  it("does not apply delayed navigation after the period changes", async () => {
    ...
    await waitFor(() =>
      expect(api.getStatistics).not.toHaveBeenCalledWith("kun", "2026-07-01"),
    );
