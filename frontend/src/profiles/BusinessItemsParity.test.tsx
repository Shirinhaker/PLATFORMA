...
describe("modular mahsulot va xizmatlar pariteti", () => {
  ...
  it("navbatli yo'nalishdagi xizmatda modular Navbat tizimi maydonini saqlaydi", async () => {
    const user = userEvent.setup();
    const create = vi.fn().mockResolvedValue(undefined);
    render(
      <StatefulItemsView
  ...
  it("modular Ombor maydonlarini yangi mahsulotda to‘liq saqlaydi", async () => {
    const user = userEvent.setup();
    const create = vi.fn().mockResolvedValue(undefined);
    render(
      <StatefulItemsView
