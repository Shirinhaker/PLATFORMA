...
function StaffFields({
  ...
  return (
    <div className="staff-modular__fields">
      <label>
        F.I.Sh.
        <input
          value={form.name}
      ...
      <label className="staff-modular__wide">
        Izoh
        <textarea
          value={form.note}
          maxLength={500}
...
function AttendanceEditor({
  ...
  return (
    <article className="staff-modular__attendance-row">
      <div>
        <b>{row.name}</b>
...
export function StaffManagement({
  ...
  if (loading) {
    return <main className="staff-modular staff-modular--message">Xodimlar yuklanmoqda…</main>;
  }
  if (!setup) {
    return (
      <main className="staff-modular staff-modular--message">
  ...
  if (screen === "create") {
    return (
      <main className="staff-modular">
        <header className="staff-modular__header">
          <button type="button" aria-label="Kabinetga qaytish" className="staff-modular__back" onClick={() => setScreen("list")}>←</button>
          <div><h1>Yangi xodim</h1><p>Asosiy ish ma’lumotlarini kiriting</p></div>
        </header>
        <section className="staff-modular__panel">
          <StaffFields
            form={form}
            professions={setup.professions}
            onChange={(patch) => setForm((current) => ({ ...current, ...patch }))}
          />
          {error && <p className="staff-modular__error" role="alert">{error}</p>}
          <button
            type="button"
  ...
  if (screen === "attendance") {
    return (
      <main className="staff-modular">
        ...
        <section className="staff-modular__panel">
          <div className="staff-modular__attendance-date">
            <label>
              Sana
              <input
                type="date"
          ...
          {attendance && !attendance.staff.length && (
            <p className="staff-modular__empty">Faol xodim yo‘q.</p>
          )}
        </section>
      </main>
    );
    ...
    return (
      <main className="staff-modular">
        <header className="staff-modular__header">
          ...
          <span className={`staff-modular__status staff-modular__status--${selected.status}`}>
            {selected.status === "active" ? "Faol" : "Ishdan bo‘shagan"}
          </span>
        </header>

        {error && <p className="staff-modular__error" role="alert">{error}</p>}
        {notice && <p className="staff-modular__notice" role="status">{notice}</p>}
        ...
        <section className="staff-modular__panel">
          ...
          <div className="staff-modular__templates">
          ...
          <div className="staff-modular__permissions">
        ...
        <section className="staff-modular__panel">
          ...
          <div className="staff-modular__schedule">
            {WEEK.map((day, index) => {
              const key = `d${index}`;
        ...
        <section className="staff-modular__danger">
  ...
  return (
    <main className="staff-modular">
      <header className="staff-modular__header">
        <button type="button" aria-label="Kabinetga qaytish" className="staff-modular__back" onClick={onBack}>←</button>
        <div><h1>Xodimlar</h1><p>Ro‘yxat, kasblar, oylik va vakolatlar</p></div>
      </header>

      <section className="staff-modular__stats">
      ...
      <div className="staff-modular__actions">
        <button
          type="button"
          onClick={() => {
        ...
        <button type="button" className="staff-modular__secondary" onClick={() => void openAttendance()}>
          📅 Ish tabeli
        </button>
      </div>

      <section className="staff-modular__profession">
        <label>
          Yangi lavozim
          <input
            value={profession}
      ...
      <section className="staff-modular__list">
        {setup.active.map((member) => (
          <button type="button" key={member.id} onClick={() => openMember(member)}>
            <span className="staff-modular__avatar">{member.name.trim().slice(0, 1).toUpperCase()}</span>
        ...
        {!setup.active.length && <p className="staff-modular__empty">Hozircha faol xodim yo‘q.</p>}
      </section>

      {setup.fired.length > 0 && (
        <details className="staff-modular__fired">
