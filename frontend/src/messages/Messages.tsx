...
export function Messages({
  ...
  const loadThread = useCallback(async (showLoading = true) => {
    ...
    try {
      ...
      setMessages((current) => {
        if (showLoading || !current.length) return result.messages;
        const merged = new Map(current.map((row) => [row.id, row]));
        for (const row of result.messages) merged.set(row.id, row);
        return [...merged.values()].sort((a, b) => a.id - b.id);
      });
      if (showLoading) setNextCursor(result.next_cursor ?? null);
  ...
  const loadOlder = useCallback(async () => {
    if (!peerKind || !peerPublicId || !nextCursor || busy) return;
    setBusy(true);
    try {
      const result = await api.getMessageThread(
        peerKind,
        peerPublicId,
        nextCursor,
      );
      setOther(result.other);
      setMessages((current) => {
        const merged = new Map(result.messages.map((row) => [row.id, row]));
  ...
  }, [api, busy, nextCursor, peerKind, peerPublicId]);
  ...
  if (!peer) {
    return (
      <main ref={rootRef} className="messages-modular">
        <header className="messages-modular__heading">
          <button type="button" onClick={onBack}>←</button>
          <h1>Suhbatlar</h1>
        </header>
        {error ? <p className="messages-modular__error" role="alert">{error}</p> : null}
  ...
  return (
    <main ref={rootRef} className="messages-modular messages-modular--thread">
      <header className="messages-modular__thread-heading">
        <button type="button" className="chat-back" onClick={leaveThread}>← Suhbatlar</button>
        <button
          type="button"
          className="messages-modular__peer"
      ...
      <section className="chat-thread" aria-label={shownName}>
        ...
        {grouped.map((group) => (
          <div className="messages-modular__day-group" key={group.day || "day"}>
      ...
      {deleteTarget ? (
        <div className="messages-modular__confirm-backdrop">
          <div className="messages-modular__confirm" role="dialog" aria-modal="true">
