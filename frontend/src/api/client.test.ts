...
describe("ApiClient", () => {
  it("uses the typed Taxi and driver endpoints with CSRF on writes", async () => {
    ...
    expect(
      fetcher.mock.calls.slice(1).map(([url, init]) => [url, init?.method, init?.body]),
    ).toEqual([
      ...
      [
        "https://api.example/api/v1/taxi/driver/available",
        "PUT",
      ...
      [
        "https://api.example/api/v1/taxi/rides/12/status",
        "POST",
        JSON.stringify({ status: "arrived" }),
      ],
      [
        "https://api.example/api/v1/taxi/rides/12/progress",
        "POST",
        JSON.stringify({ km: 1.4 }),
      ],
    ]);
  ...
  it("uses the typed K2-K4 cash register endpoints", async () => {
    ...
    expect(
      ...
      [
        "https://api.example/api/v1/cash-register/receipts",
        "POST",
        JSON.stringify(body),
      ],
      [
        "https://api.example/api/v1/cash-register/receipts/9/payment",
        "PUT",
        JSON.stringify({ pay_type: "karta" }),
      ],
  ...
  it("uses the typed K5 debt ledger and order debt payloads", async () => {
    ...
    expect(
      ...
      [
        "https://api.example/api/v1/debt-ledger/debtors",
        "POST",
        JSON.stringify(debtor),
      ],
      ["https://api.example/api/v1/debt-ledger/debtors/31", "GET", undefined],
      [
        "https://api.example/api/v1/debt-ledger/debtors/31/transactions",
        "POST",
      ...
      [
        "https://api.example/api/v1/cash-register/receipts/9/payment",
        "PUT",
        JSON.stringify({ pay_type: "qarz", debtor_id: 31 }),
      ],
      [
        "https://api.example/api/v1/orders/91/payment",
        "POST",
        JSON.stringify({ status: "debt", debtor_id: 31 }),
      ],
    ]);
  ...
  it("uses the typed K6 expense endpoints", async () => {
    ...
    expect(
      ...
      [
        "https://api.example/api/v1/expenses/categories",
        "POST",
  ...
  it("uses the typed documents and counterparties endpoints", async () => {
    ...
    expect(
      ...
      [
        "https://api.example/api/v1/documents/counterparties",
        "POST",
        JSON.stringify(counterparty),
      ],
      [
        "https://api.example/api/v1/documents/counterparties/9",
        "PUT",
        JSON.stringify(counterparty),
      ],
      ...
      [
        "https://api.example/api/v1/documents/31/send",
        "POST",
        JSON.stringify({ receiver_inn: "309333444" }),
      ],
      [
        "https://api.example/api/v1/documents/32/respond",
        "POST",
        JSON.stringify({ action: "qabul" }),
      ],
    ]);
    for (const [, init] of fetcher.mock.calls.slice(2)) {
      if (init?.method !== "GET") {
  ...
  it("uses the typed AI assistant endpoints and protects writes with csrf", async () => {
    ...
    expect(
      ...
      [
        "https://api.example/api/v1/ai-assistant/chat",
        "POST",
        JSON.stringify({ message: "Bugungi xulosa" }),
      ],
      ["https://api.example/api/v1/ai-assistant/status", "GET", undefined],
      [
        "https://api.example/api/v1/ai-assistant/documents/draft",
        "POST",
  ...
  it("uses the typed K8 statistics report and navigation endpoints", async () => {
    ...
    expect(fetcher.mock.calls.map(([url, init]) => [url, init?.method])).toEqual([
      [
        "https://api.example/api/v1/statistics" + "?period=chorak&anchor=2026-08-04",
        "GET",
      ],
      [
  ...
  it("uses the typed public queue endpoints with the exact Q3 payloads", async () => {
    ...
    expect(
      ...
      ["https://api.example/api/v1/queues", "POST", JSON.stringify(body)],
    ]);
    expect(fetcher.mock.calls[3]?.[1]?.headers).toMatchObject({
  ...
  it("uses the typed business queue endpoints with the exact Q2 payloads", async () => {
    ...
    expect(
      ...
      [
        "https://api.example/api/v1/queues/business/providers",
        "POST",
        JSON.stringify(provider),
      ],
      [
        "https://api.example/api/v1/queues/business/providers/5",
        "PUT",
        JSON.stringify(provider),
      ],
      [
        "https://api.example/api/v1/queues/business/entries?queue_date=2026-08-02",
        "GET",
        undefined,
      ],
      [
        "https://api.example/api/v1/queues/business/entries",
        "POST",
        JSON.stringify(offline),
      ],
      [
        "https://api.example/api/v1/queues/business/entries/41/status",
        "PUT",
        JSON.stringify({ status: "called" }),
      ],
      [
        "https://api.example/api/v1/queues/business/entries/41/swap",
        "POST",
        JSON.stringify({ other_queue_id: 42 }),
      ],
    ]);
    for (const [, init] of fetcher.mock.calls.slice(3)) {
      if (init?.method !== "GET") {
  ...
  it("uses the complete authenticated order flow endpoints", async () => {
    ...
    await client.sendOrderChatImage(91, {
      object_key: "orders/receipt.jpg",
    ...
    expect(
      fetcher.mock.calls.slice(1).map(([url, init]) => [url, init?.method]),
    ).toEqual([
