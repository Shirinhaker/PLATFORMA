...
import { ApiConfigurationError, loadApiBaseUrl } from "./runtime-base-url";
...
function missingConfigRequest() {
  const fetcher = vi
    ...
    .mockResolvedValueOnce(response(404, "missing"));
  return loadApiBaseUrl(fetcher as unknown as typeof fetch, location(), storage());
}
...
describe("loadApiBaseUrl", () => {
  it("uses an HTTPS query only as a non-persistent debug override", async () => {
    ...
    expect(legacyStorage.removeItem).toHaveBeenCalledWith("koprik_api_base_url");
    expect(legacyStorage.removeItem).toHaveBeenCalledWith("koprik_api_base_url_source");
  });
  ...
  it("loads the API origin from runtime-config.json", async () => {
    const fetcher = vi
      ...
      .mockResolvedValueOnce(
        response(200, JSON.stringify({ apiBaseUrl: "https://api-staging.example/" })),
      );

    await expect(
      loadApiBaseUrl(fetcher as unknown as typeof fetch, location(), storage()),
  ...
  it("uses the frontend origin when Railway requires the first-party API proxy", async () => {
    const fetcher = vi
      ...
      .mockResolvedValueOnce(
        response(200, JSON.stringify({ api_version: "v1", foundation: "phase1" })),
      );

    await expect(
      loadApiBaseUrl(fetcher as unknown as typeof fetch, location(), storage()),
    ).resolves.toBe("https://frontend-staging-production-6c41.up.railway.app");
