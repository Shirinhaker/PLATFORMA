...
function runContractOnly() {
  const frontend = fs.readFileSync(path.join(__dirname, '..', 'static', 'index.html'), 'utf8');
  for (const removed of [
    ...
    if (frontend.includes(removed)) throw new Error(`Removed home shortcut is still present: ${removed}`);
  }
  for (const required of [
    '.phone.home-active.search-results-active .screens{overflow-y:auto;',
    '.phone.home-active.search-results-active .home-discovery{order:1;',
    '.phone.home-active.search-results-active #resWrap{order:2;',
    '.phone.home-active.search-results-active #adBox{order:3;',
    '.phone.home-active.search-results-active #districtOffersMount{order:5;',
    'classList.add("search-results-active")',
    'classList.remove("search-results-active")',
  ]) {
    if (!frontend.includes(required)) throw new Error(`Mobile result-flow contract is missing: ${required}`);
  }
...
async function mockApi(page, requestState) {
  await page.addInitScript(() => {
    ...
    localStorage.setItem('koprik_home_location_v1', JSON.stringify({
      region: 'Surxondaryo viloyati',
      district: "Sho'rchi tumani",
      mahalla: '',
      lat: 37.99,
      lng: 67.79,
      exact: false,
  ...
  await page.route('**/api/**', async route => {
    ...
    if (pathname === '/api/search') return route.fulfill({ json: {
      products: [], listings: [], specialists: [], businesses: [], users: [],
      page: 1, page_size: 20, has_more: false, result_type: 'all',
    }});
...
async function verifyViewport(browser, viewport) {
  ...
  const layout = await page.evaluate(() => {
    ...
    const selectors = ['.home-search-card', '.home-map-pane', '#adBox', '#districtOffersMount'];
    return {
      ...
      shortcutCount: document.querySelectorAll('[data-web-search-type]').length,
    };
  });
  const overflow = layout.pageOverflowX;
  ...
  if (layout.shortcutCount !== 0) throw new Error(`Home shortcuts must be removed, found ${layout.shortcutCount}.`);
  if (viewport.width <= 620) {
    ...
    await page.evaluate(() => {
      enterResults('<div class="menu-card">Sinov natijasi</div>', [], 1);
    });
    await page.waitForFunction(() => document.querySelector('.phone')?.classList.contains('search-results-active'));
    const resultLayout = await page.evaluate(() => {
      const map = document.querySelector('.screen[data-screen="home"] .home-map-pane').getBoundingClientRect();
      const results = document.querySelector('#resWrap').getBoundingClientRect();
      const ad = document.querySelector('#adBox').getBoundingClientRect();
      return {
        mapHeight: Math.round(map.height),
        resultAfterMap: results.top >= map.bottom - 2,
        adAfterResults: ad.top >= results.bottom - 2,
      };
    });
    if (resultLayout.mapHeight !== 150) {
      throw new Error(`Search-state map must be 150px, received ${resultLayout.mapHeight}px.`);
    }
    if (!resultLayout.resultAfterMap || !resultLayout.adAfterResults) {
      throw new Error(`Search results order is invalid: ${JSON.stringify(resultLayout)}`);
    }
    await page.evaluate(() => exitResults());
  }
