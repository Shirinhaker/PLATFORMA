...
class SpecialistRepository:
    ...
    async def credential_count(self, session: AsyncSession, *, user_account_id: int) -> int:
        return int(await session.scalar(select(func.count(SpecialistCredential.id)).where(
    ...
    async def offer_count(self, session: AsyncSession, *, user_account_id: int) -> int:
        return int(await session.scalar(select(func.count(SpecialistOffer.id)).where(
            SpecialistOffer.user_account_id == user_account_id,
        )) or 0)

    async def portfolio_count(self, session: AsyncSession, *, user_account_id: int) -> int:
        return int(await session.scalar(select(func.count(SpecialistPortfolio.id)).where(
            SpecialistPortfolio.user_account_id == user_account_id,
        )) or 0)

    async def credential(self, session: AsyncSession, *, user_account_id: int, row_id: int, lock: bool = False):
    ...
    async def offer(self, session: AsyncSession, *, user_account_id: int, row_id: int, lock: bool = False):
    ...
    async def portfolio_item(self, session: AsyncSession, *, user_account_id: int, row_id: int, lock: bool = False):
