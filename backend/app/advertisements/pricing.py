"""Reklama narxi va jadval qoidalari.

v1656 `ad_pricing.py` dan aynan ko'chirilgan. Narx tuman-soat birligida
hisoblanadi: tanlangan tumanlar soni × kunlik soatlar × kunlar.

Bu yerda hech narsa "soddalashtirilmaydi" — narx pul degani, va eski
sayt bilan bir tiyin farq qilmasligi kerak.
"""
...
def normalize_ad_region(value):
    """Viloyat kaliti — turini bildiruvchi so'z saqlanadi.

    v1656 `normalize_ad_geo` "shahri" va "viloyati" so'zlarini olib
    tashlaydi, natijada "Toshkent shahri" va "Toshkent viloyati" bitta
    kalitga tushadi va katalogda shahar tumanlari viloyatnikiga
    almashib ketadi. Oqibati: Toshkent shahridagi tumanga reklama
    umuman sotib bo'lmaydi, viloyat tanlansa narx ham noto'g'ri
    chiqadi. Shu sababli viloyat kaliti alohida normallashtiriladi.
...
def expand_targets_to_district_keys(targets):
    ...
    for target in targets:
        ...
        if level == "republic":
            ...
            continue
        region_key = normalize_ad_region(target.get("region"))
