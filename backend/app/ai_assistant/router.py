...
async def status(current: ReadAccount, ai: Service):
    require_business_owner(current)
    return AIStatusRead(build="v1656", business_id=current.account_id, openai_enabled=ai.openai_enabled)


@router.post("/documents/draft", response_model=AIDocumentDraftRead)
async def document_draft(body: AIDocumentDraftRequest, current: WriteAccount, ai: Service):
    require_business_owner(current)
