...
class ListingRepository:
        ...
        return list(
            (
                await session.scalars(
                    statement.order_by(
                        Listing.created_at.desc(), Listing.id.desc()
                    ).limit(100)
                )
            ).all()
        )
    ...
    async def by_public_id(
        ...
        *,
        ...
        return await session.scalar(
            select(Listing).where(Listing.public_id == public_id).limit(1)
        )
        ...
        rows = list(
            (
                await session.scalars(
                    ...
                    .order_by(
                        ListingMedia.listing_id, ListingMedia.position, ListingMedia.id
                    )
                )
            ).all()
        )
    ...
    async def user_names(
        ...
        rows = (
            await session.execute(
                select(UserProfile.account_id, UserProfile.name).where(
                    UserProfile.account_id.in_(account_ids)
                )
            )
        ).all()
    ...
    async def business_names(
        ...
        rows = (
            await session.execute(
                select(BusinessProfile.account_id, BusinessProfile.name).where(
                    BusinessProfile.account_id.in_(account_ids)
                )
            )
        ).all()
        return {int(account_id): str(name) for account_id, name in rows}
