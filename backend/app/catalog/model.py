...
class CatalogGroup(Base):
    ...
    review_state: Mapped[ReviewState] = mapped_column(
        REVIEW_STATE_ENUM, nullable=False
    )
    migration_run_id: Mapped[int | None] = mapped_column(
        BigInteger, ForeignKey("migration_runs.id", ondelete="RESTRICT")
    )
...
class CatalogItem(Base):
    ...
    __table_args__ = (
        CheckConstraint("kind IN ('product', 'service')", name="ck_catalog_items_kind"),
    )
    ...
    catalog_group_id: Mapped[int | None] = mapped_column(
        BigInteger, ForeignKey("catalog_groups.id", ondelete="SET NULL")
    )
    owner_name_snapshot: Mapped[str] = mapped_column(String(160), nullable=False, default="")
    name: Mapped[str] = mapped_column(String(160), nullable=False)
    ...
    kind: Mapped[str] = mapped_column(String(20), nullable=False)
    queue_enabled: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    image_object_key: Mapped[str] = mapped_column(String(1024), nullable=False, default="")
    status: Mapped[str] = mapped_column(String(20), nullable=False)
    owner_state: Mapped[OwnerState] = mapped_column(OWNER_STATE_ENUM, nullable=False)
    review_state: Mapped[ReviewState] = mapped_column(REVIEW_STATE_ENUM, nullable=False)
    migration_run_id: Mapped[int | None] = mapped_column(
        BigInteger, ForeignKey("migration_runs.id", ondelete="RESTRICT")
    )
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
...
Index("uq_catalog_items_public_id", CatalogItem.public_id, unique=True)
Index(
...
Index("ix_catalog_items_catalog_group_id", CatalogItem.catalog_group_id)
