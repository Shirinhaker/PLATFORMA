...
from app.cash_register.schemas import CashReceiptCreate, CashSaleLineCreate
...
async def test_multi_line_receipt_updates_fifo_and_daily_totals(cash_context):
    service, engine, _sessions = cash_context
...
async def test_business_scope_permissions_dates_and_debt_guard(cash_context):
    service, _engine, _sessions = cash_context
    ...
    assert debt.value.code == "cash_debt_module_required"


async def test_order_posting_is_idempotent_and_uses_same_inventory_transaction(
    cash_context,
):
    service, engine, sessions = cash_context
    with Session(engine, expire_on_commit=False) as seed:
        order = Order(
            id=301,
