...
@pytest.mark.asyncio
async def test_reused_advertisement_moves_to_current_migration_run(store):
    db, first_run = store
    source = source_with()
    await import_advertisements(db, source, first_run)
    ...
    result = await import_advertisements(db, source, current_run)
    advertisement = (await db.scalars(select(Advertisement))).one()
    media = (await db.scalars(select(MediaMigration))).all()

    assert result.created == 0
    assert result.reused == 1
    assert advertisement.migration_run_id == current_run.id
    assert all(item.migration_run_id == current_run.id for item in media)


@pytest.mark.asyncio
async def test_reused_advertisement_refreshes_owner_after_account_split(store):
    db, first_run = store
    seed_mapping(
        db,
        first_run,
    ...
    assert advertisement.owner_user_account_id == 70
    assert advertisement.migration_run_id == current_run.id
